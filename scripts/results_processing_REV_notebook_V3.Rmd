...
for (sc in 1:length(scenarios)){
  ...
  results[scenario!="S0W0", avoided_conv_cap_mw:= (total_cap_new_conv_noRE - new_conventional_capacity_MW)]
...
positions.tech2 <- c("Battery Storage", "Solar PV", "Wind", "New Coal", "New Gas CCGT", "New Gas CT", "Existing Coal", "Existing Gas CCGT", "Existing Gas CT", "Existing Diesel", "Existing Hydro Storage", "Existing Hydro RoR", "Existing Nuclear", "Existing Other")
#Labels
...
share.labels.verbose <- c("all-wind", "more-wind", "balanced", "more-solar", "all-solar", "all-wind", "more-wind", "balanced", "more-solar", "all-solar", "all-wind", "more-wind", "balanced", "more-solar", "all-solar")
share.labels.verbose2 <- c("all wind", "more wind", "balanced", "more solar", "all solar","all wind", "more wind", "balanced", "more solar", "all solar", "all wind", "more wind", "balanced", "more solar", "all solar")
share.labels.verbose.noVRE <- c("no RE", "all-wind", "more-wind", "balanced", "more-solar", "all-solar", "all-wind", "more-wind", "balanced", "more-solar", "all-solar", "all-wind", "more-wind", "balanced", "more-solar", "all-solar")
...
plot.facet.allLines.min <- function(results.input, main_variable, scenarios.to.include, scenarios.labels, positions, plot.colors, plot.linetype, plot_title, y_limits, y_label, y_label_sec, y_sec_multiplier){
  ...
  q <- ggplot(data.plot, aes(scenario, get(main_variable), group = scenario_main, color = scenario_main, linetype = scenario_main)) +
    ...
    scale_color_manual(labels = scenarios.labels, values = plot.colors) + 
    scale_linetype_manual(labels = scenarios.labels, values = plot.linetype) +
    scale_x_discrete(breaks=positions, labels=share.labels.verbose) +
    ggtitle(plot_title) +
    theme(axis.text.x = element_text(angle=90, hjust=1, vjust=0.5), legend.position="bottom", legend.title = element_blank(), 
          legend.key = element_rect(size=5, fill = "white"), legend.key.size = unit(0.5, 'lines'), legend.key.width = unit(1, 'lines'), 
          plot.title = element_text(face="plain"), 
          strip.background = element_rect(colour="black")) +
    scale_y_continuous(limits = y_limits, sec.axis = sec_axis(~.*y_sec_multiplier, name = y_label_sec)) +
...
scenarios.to.include.multivar <- c("base",  
                              "coal_55mingen",
                              "battery30", "battery60",
                              "load_modD25M25_energyOnly", "load_modD50M0_energyOnly",
                              "wind30LC_solar30LC_coalHC")
scenarios.labels.multivar <- c("Base case", 
                          "Min gen level\ncoal fleet 55%",
...
scenarios.labels.vre <- c("base case - wind & solar levelized costs", "low solar levelized cost", "low wind levelized cost", "low wind & solar levelized costs") # "low cost wind & solar + high cost coal"
linetype_vre <- c(rep("solid", 5)) #c("solid", rep("dashed", 3), "solid", "solid")
...
## Cost of VRE
p_vre <- plot.facet.allLines.min(results.processed, "cost_vre_battery_pMWh", scenarios.to.include.vre, scenarios.labels.vre, positions, colors_RD7, linetype_vre, "Average Direct Cost of VRE", y_limits_vre_cost, "USD/MWh of VRE", "INR/kWh of VRE\n", INR_USD/1000)
p_vre + scale_linetype_manual(labels = scenarios.labels.vre, values = c("solid", rep("dashed", 3), "solid"))

## Cost of VRE uncurtailed
p_vre_noCurt <- plot.facet.allLines.min(results.processed, "cost_vre_battery_nocurt_pMWh", scenarios.to.include.vre, scenarios.labels.vre, positions, colors_RD7, linetype_vre, "Average Direct Cost of Potential VRE", y_limits_vre_cost, "USD/MWh of VRE", "INR/kWh of VRE\n", INR_USD/1000)
...
###### FIGURES FOR PAPER ######
scenarios.to.include.vre <- c("base", "base_no_curt")
scenarios.labels.vre <- c("all levelized cost scenarios", "uncurtailed VRE share")
linetype_vre2 <- c("solid", "dashed")
colors_black <- c("#000000", "#464646")

# Add vre share with curtailment 
data.plot.to.add <- results.processed[scenario!="S0W0" & scenario_main == "base", .(scenario_main, scenario, scenario_build, vre_share_after_curt)]
setnames(data.plot.to.add, c("vre_share_after_curt"), c("vre_share"))
data.plot.to.add$scenario <- reorder.factor(data.plot.to.add$scenario, new.order=positions)

# Add vre share without curtailment 
data.plot.to.add.nocurt <- results.processed[scenario!="S0W0" & scenario_main == "base", .(scenario_main, scenario, scenario_build, vre_share_nocurt)]
setnames(data.plot.to.add.nocurt, c("vre_share_nocurt"), c("vre_share"))
data.plot.to.add.nocurt[, scenario_main := "base_no_curt"]
data.plot.to.add.nocurt$scenario <- reorder.factor(data.plot.to.add.nocurt$scenario, new.order=positions)

data.plot.to.add <- rbind(data.plot.to.add, data.plot.to.add.nocurt)

## Share of VRE
p_vre_share_w_noCurt_base <- plot.facet.allLines.max(data.plot.to.add, "vre_share", scenarios.to.include.vre, scenarios.labels.vre, positions, colors_black, linetype_vre2, "VRE share of total electricity generation", y_limits_vre_share, "", "", 1)
p_vre_share_w_noCurt_base <- p_vre_share_w_noCurt_base + 
  scale_y_continuous(limits = y_limits_vre_share, labels = scales::percent_format(accuracy = 2), 
                   sec.axis = sec_axis(~.*1, name = "")) +
  theme(axis.text.y.right = element_blank())
p_vre_share_w_noCurt_base

## Capacity value of VRE uncurtailed
p_capacity_value_noCurt_base <- plot.facet.allLines.max(results.processed, "capacity_value_nocurt_pMWh", scenarios.to.include.vre[1], scenarios.labels.vre[1], positions, colors_black, linetype_vre2, "Average Capacity Value of Potential VRE", y_limits_capacity_value, "USD/MWh of VRE", "INR/kWh of VRE\n", INR_USD/1000)
p_capacity_value_noCurt_base

## Energy value of VRE uncurtailed
p_energy_value_noCurt_base <- plot.facet.allLines.max(results.processed, "energy_value_nocurt_pMWh", scenarios.to.include.vre[1], scenarios.labels.vre[1], positions, colors_black, linetype_vre2, "Average Energy Value of Potential VRE", y_limits_energy_value, "USD/MWh of VRE", "INR/kWh of VRE\n", INR_USD/1000)
p_energy_value_noCurt_base

# p_vre_share_w_noCurt2 <- p_vre_share + geom_line(data = data.plot.to.add, aes(scenario, vre_share_nocurt, group = scenario_main), color = "black", size=0.5) +
# facet_wrap(~scenario_build, scales = "free_x") +
# scale_y_continuous(limits = y_limits_vre_share, labels = scales::percent_format(accuracy = 2),
#                    sec.axis = sec_axis(~.*1, name = "")) +
#   theme(axis.text.y.right = element_blank())
...
p_vre_share_w_noCurt2 <- p_vre_share_w_noCurt + theme(legend.position = "none") + labs(tag = "C")
g_vre_share_w_noCurt2 <- ggplotGrob(p_vre_share_w_noCurt2)

p_vre_share_w_noCurt_base2 <- p_vre_share_w_noCurt_base + theme(legend.position = c(0, 0.85)) + labs(tag = "C")
g_vre_share_w_noCurt_base <- ggplotGrob(p_vre_share_w_noCurt_base2)

g_vre_noCurt <- ggplotGrob(p_vre_noCurt + theme(legend.position = "none", axis.text.x = element_blank()) + labs(tag = "A"))
g_vre_noCurt2 <- ggplotGrob(p_vre_noCurt + theme(legend.position = "none"))
g_vre_noCurt3 <- ggplotGrob(p_vre_noCurt + theme(legend.position = c(0, 0.25), axis.text.x = element_blank()) + labs(tag = "A")) 

## Modify the legend looks in this plot. 
g_capacity_value_noCurt <- ggplotGrob(p_capacity_value_noCurt + theme(legend.position = c(0, 0.65), axis.text.x = element_blank()) + labs(tag = "B")) 
g_capacity_value_noCurt2 <- ggplotGrob(p_capacity_value_noCurt + theme(legend.position = "none", axis.text.x = element_blank()))
g_capacity_value_noCurt_base <- ggplotGrob(p_capacity_value_noCurt_base + theme(legend.position = "none", axis.text.x = element_blank()) + labs(tag = "B")) 

g_energy_value_noCurt <- ggplotGrob(p_energy_value_noCurt + theme(legend.position = "none") + labs(tag = "D"))
g_energy_value_noCurt_base <- ggplotGrob(p_energy_value_noCurt_base + theme(legend.position = "none") + labs(tag = "D"))
...
#### Panel 1 is with real additional system costs of VRE
...
#### Panel 2 has only 4 plots (average VRE cost, VRE share, capacity & energy value)
grob_vre_noCurt.plot2 <- 
  arrangeGrob(grobs = 
                list(rbind(g_vre_noCurt, g_vre_share_w_noCurt2, size = "first"), 
                     rbind(g_capacity_value_noCurt, g_energy_value_noCurt, size = "last")), ncol = 2)

grid.newpage()
grid.draw(grob_vre_noCurt.plot2)

#### Panel 3 has only 4 plots (average VRE cost, VRE share, capacity & energy value) and only base/black for 3 plots [USED IN PAPER]
grob_vre_noCurt.plot3 <- 
  arrangeGrob(grobs = 
                list(rbind(g_vre_noCurt3, g_vre_share_w_noCurt_base, size = "first"), 
                     rbind(g_capacity_value_noCurt_base, g_energy_value_noCurt_base, size = "last")), ncol = 2)

grid.newpage()
grid.draw(grob_vre_noCurt.plot3)

#### Panel 4 is with % additional system costs due to VRE.
...
#### Panel 5 is with only 5 plots, legend is outside of plot, and CO2 emissions are omitted. 
...
# ggsave(file = paste0(output.folder.paper, "wind_solar_coal_costs_vre_nocurt_all_var", "_v2", ".png"), grob_vre_noCurt.plot2, width=7.5, height=5)
#ggsave(file = paste0(output.folder.paper, "wind_solar_coal_costs_vre_nocurt_all_var", "_v3", ".png"), grob_vre_noCurt.plot3, width=7.5, height=5)
#ggsave(file = paste0(output.folder.paper, "wind_solar_coal_costs_vre_nocurt_all_var", "_v3", ".pdf"), grob_vre_noCurt.plot3, width=17.8, height=11.9, units = "cm")
...
scenarios.to.include.battery <- c("base", "battery60", "battery60B50LC", "battery60B50LC_wind30LC_solar30LC")
scenarios.labels.battery <- c("base case - no battery", "battery 60 GW 4h", "low battery cost", "low battery, wind & solar costs") # "battery 30 GW 4h", "battery 60 GW 4h low cost + high cost coal"
...
p_vre_share_w_storage_w_noCurt2 <- p_vre_share_w_storage_w_noCurt + theme(legend.position = "none") + labs(tag = "C")
g_vre_share_w_storage_w_noCurt2 <- ggplotGrob(p_vre_share_w_storage_w_noCurt2)

# Levelized costs
g_vre_storage_noCurt <- ggplotGrob(p_vre_storage_noCurt + theme(legend.position = "none", axis.text.x = element_blank())  + labs(tag = "A"))
g_vre_storage_noCurt2 <- ggplotGrob(p_vre_storage_noCurt + theme(legend.position = "none"))

# Capacity value
g_capacity_value_w_storage_noCurt <- ggplotGrob(p_capacity_value_w_storage_noCurt + theme(legend.position = c(0, 0.65), axis.text.x = element_blank()) + labs(tag = "B"))
...
# Energy Value
g_energy_value_w_storage_noCurt <- ggplotGrob(p_energy_value_w_storage_noCurt + theme(legend.position = "none") + labs(tag = "D"))
...
# Net emissions mitigation costs
...
#### Panel 2 has only 4 plots (average VRE cost, VRE share, capacity & energy value)
grob_storage_noCurt.plot2 <- 
  arrangeGrob(grobs = 
                list(rbind(g_vre_storage_noCurt, g_vre_share_w_storage_w_noCurt2, size = "first"), 
                     rbind(g_capacity_value_w_storage_noCurt, g_energy_value_w_storage_noCurt, size = "last")), ncol = 2)

grid.newpage()
grid.draw(grob_storage_noCurt.plot2)
...
#ggsave(file = paste0(output.folder.paper, "battery_storage_vre_nocurt_all_var", "_v2", ".png"), grob_storage_noCurt.plot2, width=7.5, height=5)
...
scenarios.to.include.mingen <- c("base", "coal_55mingen", "coal_55mingen_solar30LC", "coal_55mingen_wind30LC_solar30LC") # "coal_55mingen_wind30LC", 
scenarios.labels.mingen <- c("base case - coal 70% min gen level", "coal 55% min gen level", "coal 55% min gen + low solar cost", "coal 55% min gen + low wind & solar costs") # "coal 55% min gen + low cost wind", 
linetype_mingen <- c(rep("solid", 6)) #c("solid", rep("dashed", 3), "solid", "solid")
...
p_vre_share_mingen_w_noCurt2 <- p_vre_share_mingen_w_noCurt + theme(legend.position = "none") + labs(tag = "C")
g_vre_share_mingen_w_noCurt2 <- ggplotGrob(p_vre_share_mingen_w_noCurt2)

# Levelized cost
g_vre_mingen_noCurt <- ggplotGrob(p_vre_mingen_noCurt + theme(legend.position = "none", axis.text.x = element_blank())+ labs(tag = "A"))
g_vre_mingen_noCurt2 <- ggplotGrob(p_vre_mingen_noCurt + theme(legend.position = "none"))

# Capacity value
g_capacity_value_mingen_noCurt <- ggplotGrob(p_capacity_value_mingen_noCurt + theme(legend.position = c(0, 0.65), axis.text.x = element_blank())+ labs(tag = "B"))
...
# Energy Value
g_energy_value_mingen_noCurt <- ggplotGrob(p_energy_value_mingen_noCurt + theme(legend.position = "none")+ labs(tag = "D"))

# Net system cost impacts
...
#### Panel 2 has only 4 plots (average VRE cost, VRE share, capacity & energy value)
grob_mingen_noCurt.plot2 <- 
  arrangeGrob(grobs = 
                list(rbind(g_vre_mingen_noCurt, g_vre_share_mingen_w_noCurt2, size = "first"), 
                     rbind(g_capacity_value_mingen_noCurt, g_energy_value_mingen_noCurt, size = "last")), ncol = 2)

grid.newpage()
grid.draw(grob_mingen_noCurt.plot2)
...
# ggsave(file = paste0(output.folder.paper, "min_gen_levels_vre_costs_nocurt_all_var", "_v2", ".png"), grob_mingen_noCurt.plot2, width=7.5, height=5)
...
### Minimum Generation Level + Battery Storage plots

```{r, minimum generation level + battery storage, echo=FALSE, message = FALSE}

scenarios.to.include.mingen_storage <- c("coal_55mingen", "coal_55mingen_battery60", "coal_55mingen_battery60B50LC", "coal_55mingen_battery60B50LC_wind30LC_solar30LC") #  "coal_55mingen_battery60_wind30LC_solar30LC"
scenarios.labels.mingen_storage <- c("coal 55% min gen level", "coal 55% min gen level + battery 60 GW 4h", "coal 55% min gen + low battery cost", "coal 55% min gen + low battery, wind & solar costs") 
linetype_mingen_storage <- c(rep("solid", 6)) #c("solid", rep("dashed", 3), "solid", "solid")


## Share of VRE
p_vre_share_mingen_storage <- plot.facet.allLines.max(results.processed, "vre_share_after_curt", scenarios.to.include.mingen_storage, scenarios.labels.mingen_storage, positions, colors_RD8, linetype_mingen_storage,  "VRE share of total electricity generation", y_limits_vre_share, "", "", INR_USD/1000)
# Add vre share without curtailment as a black line
data.plot.to.add <- results.processed[scenario!="S0W0" & scenario_main == "base", .(scenario_main, scenario, scenario_build, vre_share_nocurt)]
#setnames(data.plot, c("V4"), c(eval(main_variable)))
data.plot.to.add$scenario <- reorder.factor(data.plot.to.add$scenario, new.order=positions)

p_vre_share_mingen_storage_w_noCurt <- p_vre_share_mingen_storage + geom_line(data = data.plot.to.add, aes(scenario, vre_share_nocurt, group = scenario_main), color = "black", size=0.5) +
facet_wrap(~scenario_build, scales = "free_x") +
scale_y_continuous(limits = y_limits_vre_share, labels = scales::percent_format(accuracy = 2),
                                      sec.axis = sec_axis(~.*1, name = "")) +
  theme(axis.text.y.right = element_blank())
p_vre_share_mingen_storage_w_noCurt

## Cost of VRE and storage
p_vre_mingen_storage <- plot.facet.allLines.min(results.processed, "cost_vre_battery_pMWh", scenarios.to.include.mingen_storage, scenarios.labels.mingen_storage, positions, colors_RD8, linetype_mingen_storage,  "Average Cost of VRE and Storage", y_limits_vre_cost, "USD/MWh of VRE", "INR/kWh of VRE\n", INR_USD/1000)
p_vre_mingen_storage

## Cost of VRE uncurtailed and storage
p_vre_mingen_storage_noCurt <- plot.facet.allLines.min(results.processed, "cost_vre_battery_nocurt_pMWh", scenarios.to.include.mingen_storage, scenarios.labels.mingen_storage, positions, colors_RD8, linetype_mingen_storage, "Average Cost of Potential VRE and Storage", y_limits_vre_cost, "USD/MWh of VRE", "INR/kWh of VRE\n", INR_USD/1000)
p_vre_mingen_storage_noCurt

## Capacity value of VRE
p_capacity_value_mingen_storage <- plot.facet.allLines.max(results.processed, "capacity_value_pMWh", scenarios.to.include.mingen_storage, scenarios.labels.mingen_storage, positions, colors_RD8, linetype_mingen_storage, "Average Capacity Value of VRE and Storage", y_limits_capacity_value, "USD/MWh of VRE", "INR/kWh of VRE\n", INR_USD/1000)
p_capacity_value_mingen_storage

## Capacity value of VRE uncurtailed
p_capacity_value_mingen_storage_noCurt <- plot.facet.allLines.max(results.processed, "capacity_value_nocurt_pMWh", scenarios.to.include.mingen_storage, scenarios.labels.mingen_storage, positions, colors_RD8, linetype_mingen_storage, "Average Capacity Value of Potential VRE and Storage", y_limits_capacity_value, "USD/MWh of VRE", "INR/kWh of VRE\n", INR_USD/1000)
p_capacity_value_mingen_storage_noCurt

## Energy value of VRE
p_energy_value_mingen_storage <- plot.facet.allLines.max(results.processed, "energy_value_pMWh", scenarios.to.include.mingen_storage, scenarios.labels.mingen_storage, positions, colors_RD8, linetype_mingen_storage, "Average Energy Value of VRE and Storage", y_limits_energy_value, "USD/MWh of VRE", "INR/kWh of VRE\n", INR_USD/1000)
p_energy_value_mingen_storage

## Energy value of VRE uncurtailed
p_energy_value_mingen_storage_noCurt <- plot.facet.allLines.max(results.processed, "energy_value_nocurt_pMWh", scenarios.to.include.mingen_storage, scenarios.labels.mingen_storage, positions, colors_RD8, linetype_mingen_storage, "Average Energy Value of Potential VRE and Storage", y_limits_energy_value, "USD/MWh of VRE", "INR/kWh of VRE\n", INR_USD/1000)
p_energy_value_mingen_storage_noCurt


## System additional cost of VRE
p_sys_cost_mingen_storage <- plot.facet.allLines.min(results.processed, "system_cost_add_pMWh_load", scenarios.to.include.mingen_storage, scenarios.labels.mingen_storage, positions, colors_RD8, linetype_mingen_storage, "Average Additional Cost of VRE and Storage", y_limits_sys_cost, "USD/MWh of Load", "INR/kWh of Load\n", INR_USD/1000)
p_sys_cost_mingen_storage

## System additional cost of VRE in percentage
p_sys_cost_mingen_storage_perc <- plot.facet.allLines.min(results.processed, "system_cost_add_perc", scenarios.to.include.mingen_storage, scenarios.labels.mingen_storage, positions, colors_RD8, linetype_mingen_storage, "Additional System Cost of VRE and Storage", y_limits_sys_cost_perc, "", "", INR_USD/1000)
p_sys_cost_mingen_storage_perc <- p_sys_cost_mingen_storage_perc + scale_y_continuous(limits = y_limits_sys_cost_perc, labels = scales::percent_format(accuracy = 2), sec.axis = sec_axis(~.*1, name = "")) +
  theme(axis.text.y.right = element_blank())
p_sys_cost_mingen_storage_perc

## Cost of CO2 emissions mitigation
p_cost_emissions_avg_mingen_storage <- plot.facet.allLines.min(results.processed, "cost_emissions_reduction_pTonneCO2_avg", scenarios.to.include.mingen_storage, scenarios.labels.mingen_storage, positions, colors_RD8, linetype_mingen_storage, expression(Cost~of~CO[2]~Emissions~Mitigation), y_limits_cost_emissions, expression(USD/tonne~CO[2]), expression(INR/kg~CO[2]), INR_USD/1000)
p_cost_emissions_avg_mingen_storage

```

### Minimum generation level + Battery storage plots in a panel (cost and value expressed as per uncurtailed VRE)
Modify the min gen + battery storage plots in this code chunk

```{r, minimum generation level + battery storage panel (per uncurtailied VRE), echo=FALSE, message = FALSE}

## Using the gtable way of plotting panel data. This aligns the plots to the x and y axis when the axis labels and text vary in width or height. 

# Share
g_vre_share_mingen_storage_w_noCurt <- ggplotGrob(p_vre_share_mingen_storage_w_noCurt + theme(legend.position = "none", axis.text.x = element_blank()))

p_vre_share_mingen_storage_w_noCurt2 <- p_vre_share_mingen_storage_w_noCurt + theme(legend.position = "none") + labs(tag = "C")
g_vre_share_mingen_storage_w_noCurt2 <- ggplotGrob(p_vre_share_mingen_storage_w_noCurt2)

# Levelized costs
g_vre_mingen_storage_noCurt <- ggplotGrob(p_vre_mingen_storage_noCurt + theme(legend.position = "none", axis.text.x = element_blank()))
g_vre_mingen_storage_noCurt2 <- ggplotGrob(p_vre_mingen_storage_noCurt + theme(legend.position = "none"))

# Capacity value
g_capacity_value_mingen_storage_noCurt <- ggplotGrob(p_capacity_value_mingen_storage_noCurt + theme(legend.position = c(0, 0.65), axis.text.x = element_blank()))
g_capacity_value_mingen_storage_noCurt2 <- ggplotGrob(p_capacity_value_mingen_storage_noCurt + theme(legend.position = "none", axis.text.x = element_blank()))

# Energy value
g_energy_value_mingen_storage_noCurt <- ggplotGrob(p_energy_value_mingen_storage_noCurt + theme(legend.position = "none"))

# Net system cost impacts
g_sys_cost_mingen_storage <- ggplotGrob(p_sys_cost_mingen_storage + theme(legend.position = "none", axis.text.x = element_blank()))
g_sys_cost_mingen_storage2 <- ggplotGrob(p_sys_cost_mingen_storage + theme(legend.position = "none"))

g_sys_cost_mingen_storage_perc <- ggplotGrob(p_sys_cost_mingen_storage_perc + theme(legend.position = "none", axis.title.y.right = element_text(size=42), axis.text.x = element_blank()))

# Net emissions mitigation costs
g_cost_emissions_avg_mingen_storage <- ggplotGrob(p_cost_emissions_avg_mingen_storage + theme(legend.position = "none"))

#### Panel 1 is with real additional system costs of VRE
grob_mingen_storage_noCurt.plot <- 
  arrangeGrob(grobs = 
                list(rbind(g_vre_share_mingen_storage_w_noCurt, g_capacity_value_mingen_storage_noCurt, g_energy_value_mingen_storage_noCurt, size = "last"), 
                     rbind(g_vre_mingen_storage_noCurt, g_sys_cost_mingen_storage, g_cost_emissions_avg_mingen_storage, size = "last")), ncol = 2)

grid.newpage()
grid.draw(grob_mingen_storage_noCurt.plot)

#### Panel 2 has only 4 plots (average VRE cost, VRE share, capacity & energy value)
grob_mingen_storage_noCurt.plot2 <- 
  arrangeGrob(grobs = 
                list(rbind(g_vre_mingen_storage_noCurt, g_vre_share_mingen_storage_w_noCurt2, size = "first"), 
                     rbind(g_capacity_value_mingen_storage_noCurt, g_energy_value_mingen_storage_noCurt, size = "last")), ncol = 2)

grid.newpage()
grid.draw(grob_mingen_storage_noCurt.plot2)

#### This panel is with % additional system costs due to VRE.
grob_mingen_storage_noCurt_sys_cost_perc.plot <- 
  arrangeGrob(grobs = 
                list(rbind(g_vre_share_mingen_storage_w_noCurt, g_capacity_value_mingen_storage_noCurt, g_energy_value_mingen_storage_noCurt, size = "last"), 
                     rbind(g_vre_mingen_storage_noCurt, g_sys_cost_mingen_storage_perc, g_cost_emissions_avg_mingen_storage, size = "last")), ncol = 2)

grid.newpage()
grid.draw(grob_mingen_storage_noCurt_sys_cost_perc.plot)

#### This panel is with only 5 plots, legend is outside of plot, and CO2 emissions are omitted. 
# Grob the two legends - scenarios and black line for vre no curtailment
plot.legend.mingen_storage <- g_legend(p_capacity_value_mingen_storage_noCurt + theme(legend.position = "right", legend.margin = margin(l=2, unit='cm')))

# Use plot legend from vre cost panel
# plot.legend.vre.nocurt <- g_legend(p_vre_share_noCurt + theme(legend.position = "right", legend.margin = margin(l=2, unit='cm')))

# First column plot
grob_mingen_storage_noCurt.5plot.1 <-
  arrangeGrob(grobs =
                list(rbind(g_vre_share_mingen_storage_w_noCurt, g_capacity_value_mingen_storage_noCurt2, g_energy_value_mingen_storage_noCurt, size = "last")), ncol = 1)

# Second column plot
grob_mingen_storage_noCurt.5plot.2 <-
  arrangeGrob(grobs =
                list(rbind(g_vre_mingen_storage_noCurt, g_sys_cost_mingen_storage2, size = "last")), ncol = 1)

grob_plot_legend_mingen_storage_nocurt <- plot_grid(plot.legend.vre.nocurt, plot.legend.mingen_storage, nrow = 2, rel_heights = c(1,4))

grob_mingen_storage_noCurt.5plot.2.legend <- plot_grid(grob_plot_legend_mingen_storage_nocurt,
                                                              grob_mingen_storage_noCurt.5plot.2,
                                                              nrow = 2, rel_heights = c(1,2.6))

grob_mingen_storage_noCurt.5plot.3 <- plot_grid(grob_mingen_storage_noCurt.5plot.1,
                                                   grob_mingen_storage_noCurt.5plot.2.legend,
                                                   ncol = 2, rel_widths = c(1,1.02))

grob_mingen_storage_noCurt.5plot.3

#ggsave(file = paste0(output.folder.paper, "min_gen_storage_vre_costs_nocurt_all_var", "_v1", ".png"), grob_mingen_storage_noCurt.plot, width=7, height=5)
#ggsave(file = paste0(output.folder.paper, "min_gen_storage_vre_costs_nocurt_all_var", "_v2", ".png"), grob_mingen_storage_noCurt.plot2, width=7.5, height=5)
# ggsave(file = paste0(output.folder.paper, "min_gen_storage_vre_costs_nocurt_all_var_sys_cost_perc", "_v1", ".png"), grob_mingen_storage_noCurt_sys_cost_perc.plot, width=7, height=5)
# ggsave(file = paste0(output.folder.paper, "min_gen_storage_vre_noCurt_5_var_sys_cost_perc", "_v1", ".png"), grob_mingen_storage_noCurt.5plot.3, width=7, height=5)
# ggsave(file = paste0(output.folder.paper, "min_gen_storage_vre_costs_noCurt_5_var", "_v1", ".png"), grob_mingen_storage_noCurt.5plot.3, width=7, height=5)
```
...
### Low demand
scenarios.to.include.loadMod <- c("base", "load_mod_low15_energyOnly")
scenarios.labels.loadMod <- c("base case forecast-India 100%", "forecast-15% lower demand")
...
p_vre_share_loadMod1 <- p_vre_share_loadMod +
  scale_y_continuous(limits = y_limits_vre_share, labels = scales::percent_format(accuracy = 2),
                                      sec.axis = sec_axis(~.*1, name = "")) +
  theme(axis.text.y.right = element_blank())
p_vre_share_loadMod1
...
p_vre_share_loadMod_w_noCurt2 <- p_vre_share_loadMod_w_noCurt + theme(legend.position = "none") + labs(tag = "C")
g_vre_share_loadMod_w_noCurt2 <- ggplotGrob(p_vre_share_loadMod_w_noCurt2)

p_vre_share_loadMod2 <- p_vre_share_loadMod1 + theme(legend.position = "none") + labs(tag = "C")
g_vre_share_loadMod2 <- ggplotGrob(p_vre_share_loadMod2)

# Levelized costs
g_vre_loadMod_noCurt <- ggplotGrob(p_vre_loadMod_noCurt + theme(legend.position = "none", axis.text.x = element_blank()) + labs(tag = "A"))
g_vre_loadMod_noCurt2 <- ggplotGrob(p_vre_loadMod_noCurt + theme(legend.position = "none"))

# Capacity value
g_capacity_value_loadMod_noCurt <- ggplotGrob(p_capacity_value_loadMod_noCurt + theme(legend.position = c(0, 0.65), axis.text.x = element_blank()) + labs(tag = "B"))
...
# Energy value
g_energy_value_loadMod_noCurt <- ggplotGrob(p_energy_value_loadMod_noCurt + theme(legend.position = "none") + labs(tag = "D"))
...
#### Panel 2 has only 4 plots (average VRE cost, VRE share, capacity & energy value)
grob_loadMod_noCurt.plot2 <- 
  arrangeGrob(grobs = 
                list(rbind(g_vre_loadMod_noCurt, g_vre_share_loadMod2, size = "first"), 
                     rbind(g_capacity_value_loadMod_noCurt, g_energy_value_loadMod_noCurt, size = "last")), ncol = 2)

grid.newpage()
grid.draw(grob_loadMod_noCurt.plot2)

#### Panel 3 - This panel is with % additional system costs due to VRE.
...
#### Panel 4 - This panel is with only 5 plots, legend is outside of plot, and CO2 emissions are omitted. 
# Grob the two legends - scenarios and black line for vre no curtailment
...
#### Panel 5 is for net system costs and emissions costs

p_cost_emissions_avg_loadMod1 <- p_cost_emissions_avg_loadMod + ggtitle("Carbon Emissions Mitigation Costs") + 
                                     theme(legend.position = c(0, 1), legend.justification = c(0, 1))

g_cost_emissions_avg_loadMod2 <- ggplotGrob(p_cost_emissions_avg_loadMod1 + labs(tag = "A")) 

# g_cost_emissions_avg_base2 <- ggplotGrob(p_cost_emissions_avg1 + 
#                                            theme(axis.text.x = element_blank()) + 
#                                            labs(tag = "A")) 

g_sys_cost_loadMod3 <- ggplotGrob(p_sys_cost_loadMod + ggtitle("Net System Costs") + 
                           theme(legend.position = "none") +
                            labs(tag = "B"))

grob_cost_emissions_avg_sysCost_loadMod.plot <- 
  arrangeGrob(grobs = 
                list(rbind(g_cost_emissions_avg_loadMod2, size = "last"), 
                     rbind(g_sys_cost_loadMod3, size = "last")), ncol = 2)

grid.newpage()
grid.draw(grob_cost_emissions_avg_sysCost_loadMod.plot)
...
# ggsave(file = paste0(output.folder.paper, "modified_loads_vre_nocurt_all_var", "_v2", ".png"), grob_loadMod_noCurt.plot2, width=7.5, height=5)
...
# ggsave(file = paste0(output.folder.paper, "low_load_emissions_and_net_sys_costs_levcost_scenarios", "_v1", ".png"), grob_cost_emissions_avg_sysCost_loadMod.plot, width=7, height=3)
...
### High cost coal plots

```{r, high cost coal, echo=FALSE, message = FALSE}

scenarios.to.include.coalhc <- c("base", "high_cost_coal", "wind30LC_solar30LC_coalHC")
scenarios.labels.coalhc <- c("base case - low coal costs", "high coal costs", "high coal costs & low wind & solar costs")
linetype_coalhc <- c(rep("solid", 5)) #c("solid", "dashed", "dashed")

## Share of VRE
p_vre_share_coalhc <- plot.facet.allLines.max(results.processed, "vre_share_after_curt", scenarios.to.include.coalhc, scenarios.labels.coalhc, positions, colors_RD8, linetype_coalhc,  "VRE share of total electricity generation", y_limits_vre_share, "", "", INR_USD/1000)
# Add vre share without curtailment as a black line
data.plot.to.add <- results.processed[scenario!="S0W0" & scenario_main == "base", .(scenario_main, scenario, scenario_build, vre_share_nocurt)]
#setnames(data.plot, c("V4"), c(eval(main_variable)))
data.plot.to.add$scenario <- reorder.factor(data.plot.to.add$scenario, new.order=positions)

p_vre_share_coalhc_w_noCurt <- p_vre_share_coalhc + geom_line(data = data.plot.to.add, aes(scenario, vre_share_nocurt, group = scenario_main), color = "black", size=0.5) +
facet_wrap(~scenario_build, scales = "free_x") +
scale_y_continuous(limits = y_limits_vre_share, labels = scales::percent_format(accuracy = 2),
                                      sec.axis = sec_axis(~.*1, name = "")) +
  theme(axis.text.y.right = element_blank())
p_vre_share_coalhc_w_noCurt

## Cost of VRE and storage
p_vre_coalhc <- plot.facet.allLines.min(results.processed, "cost_vre_battery_pMWh", scenarios.to.include.coalhc, scenarios.labels.coalhc, positions, colors_RD8, linetype_coalhc,  "Average Cost of VRE", y_limits_vre_cost, "USD/MWh of VRE", "INR/kWh of VRE\n", INR_USD/1000)
p_vre_coalhc

## Cost of VRE uncurtailed and storage
p_vre_coalhc_noCurt <- plot.facet.allLines.min(results.processed, "cost_vre_battery_nocurt_pMWh", scenarios.to.include.coalhc, scenarios.labels.coalhc, positions, colors_RD8, linetype_coalhc, "Average Cost of Potential VRE", y_limits_vre_cost, "USD/MWh of VRE", "INR/kWh of VRE\n", INR_USD/1000)
p_vre_coalhc_noCurt

## Capacity value of VRE
p_capacity_value_coalhc <- plot.facet.allLines.max(results.processed, "capacity_value_pMWh", scenarios.to.include.coalhc, scenarios.labels.coalhc, positions, colors_RD8, linetype_coalhc, "Average Capacity Value of VRE", y_limits_capacity_value, "USD/MWh of VRE", "INR/kWh of VRE\n", INR_USD/1000)
p_capacity_value_coalhc

## Capacity value of VRE uncurtailed
p_capacity_value_coalhc_noCurt <- plot.facet.allLines.max(results.processed, "capacity_value_nocurt_pMWh", scenarios.to.include.coalhc, scenarios.labels.coalhc, positions, colors_RD8, linetype_coalhc, "Average Capacity Value of Potential VRE", y_limits_capacity_value, "USD/MWh of VRE", "INR/kWh of VRE\n", INR_USD/1000)
p_capacity_value_coalhc_noCurt

## Energy value of VRE
p_energy_value_coalhc <- plot.facet.allLines.max(results.processed, "energy_value_pMWh", scenarios.to.include.coalhc, scenarios.labels.coalhc, positions, colors_RD8, linetype_coalhc, "Average Energy Value of VRE", y_limits_energy_value, "USD/MWh of VRE", "INR/kWh of VRE\n", INR_USD/1000)
p_energy_value_coalhc

## Energy value of VRE uncurtailed
p_energy_value_coalhc_noCurt <- plot.facet.allLines.max(results.processed, "energy_value_nocurt_pMWh", scenarios.to.include.coalhc, scenarios.labels.coalhc, positions, colors_RD8, linetype_coalhc, "Average Energy Value of Potential VRE", y_limits_energy_value, "USD/MWh of VRE", "INR/kWh of VRE\n", INR_USD/1000)
p_energy_value_coalhc_noCurt

## System additional cost of VRE
p_sys_cost_coalhc <- plot.facet.allLines.min(results.processed, "system_cost_add_pMWh_load", scenarios.to.include.coalhc, scenarios.labels.coalhc, positions, colors_RD8, linetype_coalhc, "Average Additional Cost of VRE", y_limits_sys_cost, "USD/MWh of Load", "INR/kWh of Load\n", INR_USD/1000)
p_sys_cost_coalhc

## System additional cost of VRE in percentage
p_sys_cost_coalhc_perc <- plot.facet.allLines.min(results.processed, "system_cost_add_perc", scenarios.to.include.coalhc, scenarios.labels.coalhc, positions, colors_RD8, linetype_coalhc, "Additional System Cost of VRE", y_limits_sys_cost_perc, "", "", INR_USD/1000)
p_sys_cost_coalhc_perc <- p_sys_cost_coalhc_perc + scale_y_continuous(limits = y_limits_sys_cost_perc, labels = scales::percent_format(accuracy = 2), sec.axis = sec_axis(~.*1, name = "")) +
  theme(axis.text.y.right = element_blank())
p_sys_cost_coalhc_perc

## Cost of CO2 emissions mitigation
p_cost_emissions_avg_coalhc <- plot.facet.allLines.min(results.processed, "cost_emissions_reduction_pTonneCO2_avg", scenarios.to.include.coalhc, scenarios.labels.coalhc, positions, colors_RD8, linetype_coalhc, expression(Cost~of~CO[2]~Emissions~Mitigation), y_limits_cost_emissions, expression(USD/tonne~CO[2]), expression(INR/kg~CO[2]), INR_USD/1000)
p_cost_emissions_avg_coalhc

```


### High cost coal plots in a panel (cost and value expressed as per uncurtailed VRE)
Modify the high cost coal plots in this code chunk
...
# Share
g_vre_share_coalhc_w_noCurt <- ggplotGrob(p_vre_share_coalhc_w_noCurt + theme(legend.position = "none", axis.text.x = element_blank()))

p_vre_share_coalhc_w_noCurt2 <- p_vre_share_coalhc_w_noCurt + theme(legend.position = "none") + labs(tag = "C")
g_vre_share_coalhc_w_noCurt2 <- ggplotGrob(p_vre_share_coalhc_w_noCurt2)

# Levelized costs
g_vre_coalhc_noCurt <- ggplotGrob(p_vre_coalhc_noCurt + theme(legend.position = "none", axis.text.x = element_blank()) + labs(tag = "A"))
g_vre_coalhc_noCurt2 <- ggplotGrob(p_vre_coalhc_noCurt + theme(legend.position = "none"))

# Capacity value
g_capacity_value_coalhc_noCurt <- ggplotGrob(p_capacity_value_coalhc_noCurt + theme(legend.position = c(0, 0.65), axis.text.x = element_blank()) + labs(tag = "B"))
g_capacity_value_coalhc_noCurt2 <- ggplotGrob(p_capacity_value_coalhc_noCurt + theme(legend.position = "none", axis.text.x = element_blank()))

# Energy value
g_energy_value_coalhc_noCurt <- ggplotGrob(p_energy_value_coalhc_noCurt + theme(legend.position = "none") + labs(tag = "D"))

# Net system cost impacts
g_sys_cost_coalhc <- ggplotGrob(p_sys_cost_coalhc + theme(legend.position = "none", axis.text.x = element_blank()))
g_sys_cost_coalhc2 <- ggplotGrob(p_sys_cost_coalhc + theme(legend.position = "none"))

g_sys_cost_coalhc_perc <- ggplotGrob(p_sys_cost_coalhc_perc + theme(legend.position = "none", axis.title.y.right = element_text(size=42), axis.text.x = element_blank()))

# Net emissions mitigation costs
g_cost_emissions_avg_coalhc <- ggplotGrob(p_cost_emissions_avg_coalhc + theme(legend.position = "none"))

#### Panel 1 is with real additional system costs of VRE
grob_coalhc_noCurt.plot <- 
  arrangeGrob(grobs = 
                list(rbind(g_vre_share_coalhc_w_noCurt, g_capacity_value_coalhc_noCurt, g_energy_value_coalhc_noCurt, size = "last"), 
                     rbind(g_vre_coalhc_noCurt, g_sys_cost_coalhc, g_cost_emissions_avg_coalhc, size = "last")), ncol = 2)

grid.newpage()
grid.draw(grob_coalhc_noCurt.plot)

#### Panel 2 has only 4 plots (average VRE cost, VRE share, capacity & energy value)
grob_coalhc_noCurt.plot2 <- 
  arrangeGrob(grobs = 
                list(rbind(g_vre_coalhc_noCurt, g_vre_share_coalhc_w_noCurt2, size = "first"), 
                     rbind(g_capacity_value_coalhc_noCurt, g_energy_value_coalhc_noCurt, size = "last")), ncol = 2)

grid.newpage()
grid.draw(grob_coalhc_noCurt.plot2)

#### This panel is with % additional system costs due to VRE.
grob_coalhc_noCurt_sys_cost_perc.plot <- 
  arrangeGrob(grobs = 
                list(rbind(g_vre_share_coalhc_w_noCurt, g_capacity_value_coalhc_noCurt, g_energy_value_coalhc_noCurt, size = "last"), 
                     rbind(g_vre_coalhc_noCurt, g_sys_cost_coalhc_perc, g_cost_emissions_avg_coalhc, size = "last")), ncol = 2)

grid.newpage()
grid.draw(grob_coalhc_noCurt_sys_cost_perc.plot)

#### This panel is with only 5 plots, legend is outside of plot, and CO2 emissions are omitted. 
# Grob the two legends - scenarios and black line for vre no curtailment
plot.legend.coalhc <- g_legend(p_capacity_value_coalhc_noCurt + theme(legend.position = "right", legend.margin = margin(l=2, unit='cm')))

# Use plot legend from vre cost panel
# plot.legend.vre.nocurt <- g_legend(p_vre_share_noCurt + theme(legend.position = "right", legend.margin = margin(l=2, unit='cm')))

# First column plot
grob_coalhc_noCurt.5plot.1 <-
  arrangeGrob(grobs =
                list(rbind(g_vre_share_coalhc_w_noCurt, g_capacity_value_coalhc_noCurt2, g_energy_value_coalhc_noCurt, size = "last")), ncol = 1)

# Second column plot
grob_coalhc_noCurt.5plot.2 <-
  arrangeGrob(grobs =
                list(rbind(g_vre_coalhc_noCurt, g_sys_cost_coalhc2, size = "last")), ncol = 1)

grob_plot_legend_coalhc_nocurt <- plot_grid(plot.legend.vre.nocurt, plot.legend.coalhc, nrow = 2, rel_heights = c(1,4))

grob_coalhc_noCurt.5plot.2.legend <- plot_grid(grob_plot_legend_coalhc_nocurt,
                                                              grob_coalhc_noCurt.5plot.2,
                                                              nrow = 2, rel_heights = c(1,2.6))

grob_coalhc_noCurt.5plot.3 <- plot_grid(grob_coalhc_noCurt.5plot.1,
                                                   grob_coalhc_noCurt.5plot.2.legend,
                                                   ncol = 2, rel_widths = c(1,1.02))

grob_coalhc_noCurt.5plot.3

# ggsave(file = paste0(output.folder.paper, "coal_high_cost_vre_nocurt_all_var", "_v1", ".png"), grob_coalhc_noCurt.plot, width=7, height=5)
# ggsave(file = paste0(output.folder.paper, "coal_high_cost_vre_nocurt_all_var", "_v2", ".png"), grob_coalhc_noCurt.plot2, width=7.5, height=5)
# ggsave(file = paste0(output.folder.paper, "coal_high_cost_vre_nocurt_all_var_sys_cost_perc", "_v1", ".png"), grob_coalhc_noCurt_sys_cost_perc.plot, width=7, height=5)
# ggsave(file = paste0(output.folder.paper, "coal_high_cost_vre_noCurt_5_var", "_v1", ".png"), grob_coalhc_noCurt.5plot.3, width=7, height=5)

```

### High hydro capacity and nuclear

```{r, high hydro and nuclear, echo=FALSE, message = FALSE}

scenarios.to.include.hydro.nuclear <- c("base", "hydro_high_cea", "nuclear17", "hydro_high_cea_nuclear17")
scenarios.labels.hydro.nuclear <- c("base case - no new hydro or nuclear", "high hydro buildout", "high nuclear buildout", "high hydro and nuclear buildout")
linetype_hydro_nuclear <- c(rep("solid", 5)) #c("solid", "dashed", "dashed")

## Share of VRE
p_vre_share_hydro_nuclear <- plot.facet.allLines.max(results.processed, "vre_share_after_curt", scenarios.to.include.hydro.nuclear, scenarios.labels.hydro.nuclear, positions, colors_RD8, linetype_hydro_nuclear,  "VRE share of total electricity generation", y_limits_vre_share, "", "", INR_USD/1000)
# Add vre share without curtailment as a black line
data.plot.to.add <- results.processed[scenario!="S0W0" & scenario_main == "base", .(scenario_main, scenario, scenario_build, vre_share_nocurt)]
#setnames(data.plot, c("V4"), c(eval(main_variable)))
data.plot.to.add$scenario <- reorder.factor(data.plot.to.add$scenario, new.order=positions)

p_vre_share_hydro_nuclear_w_noCurt <- p_vre_share_hydro_nuclear + geom_line(data = data.plot.to.add, aes(scenario, vre_share_nocurt, group = scenario_main), color = "black", size=0.5) +
facet_wrap(~scenario_build, scales = "free_x") +
scale_y_continuous(limits = y_limits_vre_share, labels = scales::percent_format(accuracy = 2),
                                      sec.axis = sec_axis(~.*1, name = "")) +
  theme(axis.text.y.right = element_blank())
p_vre_share_hydro_nuclear_w_noCurt

## Cost of VRE and storage
p_vre_hydro_nuclear <- plot.facet.allLines.min(results.processed, "cost_vre_battery_pMWh", scenarios.to.include.hydro.nuclear, scenarios.labels.hydro.nuclear, positions, colors_RD8, linetype_hydro_nuclear,  "Average Cost of VRE", y_limits_vre_cost, "USD/MWh of VRE", "INR/kWh of VRE\n", INR_USD/1000)
p_vre_hydro_nuclear

## Cost of VRE uncurtailed and storage
p_vre_hydro_nuclear_noCurt <- plot.facet.allLines.min(results.processed, "cost_vre_battery_nocurt_pMWh", scenarios.to.include.hydro.nuclear, scenarios.labels.hydro.nuclear, positions, colors_RD8, linetype_hydro_nuclear, "Average Cost of Potential VRE", y_limits_vre_cost, "USD/MWh of VRE", "INR/kWh of VRE\n", INR_USD/1000)
p_vre_hydro_nuclear_noCurt

## Capacity value of VRE
p_capacity_value_hydro_nuclear <- plot.facet.allLines.max(results.processed, "capacity_value_pMWh", scenarios.to.include.hydro.nuclear, scenarios.labels.hydro.nuclear, positions, colors_RD8, linetype_hydro_nuclear, "Average Capacity Value of VRE", y_limits_capacity_value, "USD/MWh of VRE", "INR/kWh of VRE\n", INR_USD/1000)
p_capacity_value_hydro_nuclear

## Capacity value of VRE uncurtailed
p_capacity_value_hydro_nuclear_noCurt <- plot.facet.allLines.max(results.processed, "capacity_value_nocurt_pMWh", scenarios.to.include.hydro.nuclear, scenarios.labels.hydro.nuclear, positions, colors_RD8, linetype_hydro_nuclear, "Average Capacity Value of Potential VRE", y_limits_capacity_value, "USD/MWh of VRE", "INR/kWh of VRE\n", INR_USD/1000)
p_capacity_value_hydro_nuclear_noCurt

## Energy value of VRE
p_energy_value_hydro_nuclear <- plot.facet.allLines.max(results.processed, "energy_value_pMWh", scenarios.to.include.hydro.nuclear, scenarios.labels.hydro.nuclear, positions, colors_RD8, linetype_hydro_nuclear, "Average Energy Value of VRE", y_limits_energy_value, "USD/MWh of VRE", "INR/kWh of VRE\n", INR_USD/1000)
p_energy_value_hydro_nuclear

## Energy value of VRE uncurtailed
p_energy_value_hydro_nuclear_noCurt <- plot.facet.allLines.max(results.processed, "energy_value_nocurt_pMWh", scenarios.to.include.hydro.nuclear, scenarios.labels.hydro.nuclear, positions, colors_RD8, linetype_hydro_nuclear, "Average Energy Value of Potential VRE", y_limits_energy_value, "USD/MWh of VRE", "INR/kWh of VRE\n", INR_USD/1000)
p_energy_value_hydro_nuclear_noCurt

## System additional cost of VRE
p_sys_cost_hydro_nuclear <- plot.facet.allLines.min(results.processed, "system_cost_add_pMWh_load", scenarios.to.include.hydro.nuclear, scenarios.labels.hydro.nuclear, positions, colors_RD8, linetype_hydro_nuclear, "Average Additional Cost of VRE", y_limits_sys_cost, "USD/MWh of Load", "INR/kWh of Load\n", INR_USD/1000)
p_sys_cost_hydro_nuclear

## System additional cost of VRE in percentage
p_sys_cost_hydro_nuclear_perc <- plot.facet.allLines.min(results.processed, "system_cost_add_perc", scenarios.to.include.hydro.nuclear, scenarios.labels.hydro.nuclear, positions, colors_RD8, linetype_hydro_nuclear, "Additional System Cost of VRE", y_limits_sys_cost_perc, "", "", INR_USD/1000)
p_sys_cost_hydro_nuclear_perc <- p_sys_cost_hydro_nuclear_perc + scale_y_continuous(limits = y_limits_sys_cost_perc, labels = scales::percent_format(accuracy = 2), sec.axis = sec_axis(~.*1, name = "")) +
  theme(axis.text.y.right = element_blank())
p_sys_cost_hydro_nuclear_perc

## Cost of CO2 emissions mitigation
p_cost_emissions_avg_hydro_nuclear <- plot.facet.allLines.min(results.processed, "cost_emissions_reduction_pTonneCO2_avg", scenarios.to.include.hydro.nuclear, scenarios.labels.hydro.nuclear, positions, colors_RD8, linetype_hydro_nuclear, expression(Cost~of~CO[2]~Emissions~Mitigation), y_limits_cost_emissions, expression(USD/tonne~CO[2]), expression(INR/kg~CO[2]), INR_USD/1000)
p_cost_emissions_avg_hydro_nuclear

```

### High hydro and nuclear plots in a panel (cost and value expressed as per uncurtailed VRE)

```{r, high hydro and nuclear panel (per uncurtailed VRE), echo=FALSE, message = FALSE}

## Using the gtable way of plotting panel data. This aligns the plots to the x and y axis when the axis labels and text vary in width or height. 

# Share
g_vre_share_hydro_nuclear_w_noCurt <- ggplotGrob(p_vre_share_hydro_nuclear_w_noCurt + theme(legend.position = "none", axis.text.x = element_blank()))

p_vre_share_hydro_nuclear_w_noCurt2 <- p_vre_share_hydro_nuclear_w_noCurt + theme(legend.position = "none") + labs(tag = "C")
g_vre_share_hydro_nuclear_w_noCurt2 <- ggplotGrob(p_vre_share_hydro_nuclear_w_noCurt2)

# Levelized costs
g_vre_hydro_nuclear_noCurt <- ggplotGrob(p_vre_hydro_nuclear_noCurt + theme(legend.position = "none", axis.text.x = element_blank()) + labs(tag = "A"))
g_vre_hydro_nuclear_noCurt2 <- ggplotGrob(p_vre_hydro_nuclear_noCurt + theme(legend.position = "none"))

# Capacity value
g_capacity_value_hydro_nuclear_noCurt <- ggplotGrob(p_capacity_value_hydro_nuclear_noCurt + theme(legend.position = c(0, 0.65), axis.text.x = element_blank()) + labs(tag = "B"))
g_capacity_value_hydro_nuclear_noCurt2 <- ggplotGrob(p_capacity_value_hydro_nuclear_noCurt + theme(legend.position = "none", axis.text.x = element_blank()))

# Energy value
g_energy_value_hydro_nuclear_noCurt <- ggplotGrob(p_energy_value_hydro_nuclear_noCurt + theme(legend.position = "none") + labs(tag = "D"))

# Net system cost impacts
g_sys_cost_hydro_nuclear <- ggplotGrob(p_sys_cost_hydro_nuclear + theme(legend.position = "none", axis.text.x = element_blank()))
g_sys_cost_hydro_nuclear2 <- ggplotGrob(p_sys_cost_hydro_nuclear + theme(legend.position = "none"))

g_sys_cost_hydro_nuclear_perc <- ggplotGrob(p_sys_cost_hydro_nuclear_perc + theme(legend.position = "none", axis.title.y.right = element_text(size=42), axis.text.x = element_blank()))

# Net emissions mitigation costs
g_cost_emissions_avg_hydro_nuclear <- ggplotGrob(p_cost_emissions_avg_hydro_nuclear + theme(legend.position = "none"))

#### Panel 1 is with real additional system costs of VRE
grob_hydro_nuclear_noCurt.plot <-
  arrangeGrob(grobs =
                list(rbind(g_vre_share_hydro_nuclear_w_noCurt, g_capacity_value_hydro_nuclear_noCurt, g_energy_value_hydro_nuclear_noCurt, size = "last"),
                     rbind(g_vre_hydro_nuclear_noCurt, g_sys_cost_hydro_nuclear, g_cost_emissions_avg_hydro_nuclear, size = "last")), ncol = 2)

grid.newpage()
grid.draw(grob_hydro_nuclear_noCurt.plot)

#### Panel 2 has only 4 plots (average VRE cost, VRE share, capacity & energy value)
grob_hydro_nuclear_noCurt.plot2 <- 
  arrangeGrob(grobs = 
                list(rbind(g_vre_hydro_nuclear_noCurt, g_vre_share_hydro_nuclear_w_noCurt2, size = "first"), 
                     rbind(g_capacity_value_hydro_nuclear_noCurt, g_energy_value_hydro_nuclear_noCurt, size = "last")), ncol = 2)

grid.newpage()
grid.draw(grob_hydro_nuclear_noCurt.plot2)

#### Panel 3 is for net system costs and emissions costs

p_cost_emissions_avg_hydro_nuclear1 <- p_cost_emissions_avg_hydro_nuclear + ggtitle("Carbon Emissions Mitigation Costs") + 
                                     theme(legend.position = c(0, 1), legend.justification = c(0, 1))

g_cost_emissions_avg_hydro_nuclear2 <- ggplotGrob(p_cost_emissions_avg_hydro_nuclear1 + labs(tag = "A")) 

# g_cost_emissions_avg_base2 <- ggplotGrob(p_cost_emissions_avg1 + 
#                                            theme(axis.text.x = element_blank()) + 
#                                            labs(tag = "A")) 

g_sys_cost_hydro_nuclear3 <- ggplotGrob(p_sys_cost_hydro_nuclear + ggtitle("Net System Costs") + 
                           theme(legend.position = "none") +
                            labs(tag = "B"))

grob_cost_emissions_avg_sysCost_hydro_nuclear.plot <- 
  arrangeGrob(grobs = 
                list(rbind(g_cost_emissions_avg_hydro_nuclear2, size = "last"), 
                     rbind(g_sys_cost_hydro_nuclear3, size = "last")), ncol = 2)

grid.newpage()
grid.draw(grob_cost_emissions_avg_sysCost_hydro_nuclear.plot)



# ggsave(file = paste0(output.folder.paper, "coal_high_cost_vre_nocurt_all_var", "_v1", ".png"), grob_coalhc_noCurt.plot, width=7, height=5)
# ggsave(file = paste0(output.folder.paper, "high_hydro_nuclear_vre_nocurt_all_var", "_v1", ".png"), grob_hydro_nuclear_noCurt.plot2, width=7.5, height=5)
# ggsave(file = paste0(output.folder.paper, "high_hydro_nuclear_emissions_and_net_sys_costs_levcost_scenarios", "_v1", ".png"), grob_cost_emissions_avg_sysCost_hydro_nuclear.plot, width=7, height=3)
```


###  Average Emissions Mitigation Costs AND Additional System Costs due to VRE (and Storage) Panel for base case

```{r, avg emissions and additional system costs panel, echo=FALSE, message = FALSE}

p_cost_emissions_avg1 <- p_cost_emissions_avg + ggtitle("Carbon Emissions Mitigation Costs") + 
                                     theme(legend.position = c(0, 1), legend.justification = c(0, 1))

g_cost_emissions_avg_base <- ggplotGrob(p_cost_emissions_avg1 + labs(tag = "A")) 

g_cost_emissions_avg_base2 <- ggplotGrob(p_cost_emissions_avg1 + 
                                           theme(axis.text.x = element_blank()) + 
                                           labs(tag = "A")) 

g_sys_cost_base <- ggplotGrob(p_sys_cost + ggtitle("Net System Costs") + 
                           theme(legend.position = "none") +
                            labs(tag = "B"))

grob_cost_emissions_avg_sysCost.plot <- 
  arrangeGrob(grobs = 
                list(rbind(g_cost_emissions_avg_base, size = "last"), 
                     rbind(g_sys_cost_base, size = "last")), ncol = 2)

grid.newpage()
grid.draw(grob_cost_emissions_avg_sysCost.plot)

grob_cost_emissions_avg_sysCost.plot2 <- 
  arrangeGrob(grobs = 
                list(rbind(g_cost_emissions_avg_base2, g_sys_cost_base, size = "first")), ncol = 1)

grid.newpage()
grid.draw(grob_cost_emissions_avg_sysCost.plot2)

#ggsave(file = paste0(output.folder.paper, "emissions_and_net_sys_costs_levcost_scenarios", "_v1", ".png"), grob_cost_emissions_avg_sysCost.plot, width=7, height=3)
#ggsave(file = paste0(output.folder.paper, "emissions_and_net_sys_costs_levcost_scenarios", "_v2", ".pdf"), grob_cost_emissions_avg_sysCost.plot2, width=8.7, height=13, units = "cm")
...
g_sys_cost <- ggplotGrob(p_sys_cost + ggtitle("Wind and solar costs") + 
                           theme(legend.position = c(0, 1), legend.justification = c(0, 1), axis.text.x = element_blank()) +
                            labs(tag = "A"))

g_sys_cost_mingen <- ggplotGrob(p_sys_cost_mingen + ggtitle("Lower coal minimum generation level") + 
                                  theme(legend.position = c(0, 1), legend.justification = c(0, 1), 
                                        axis.text.x = element_blank()) + labs(tag = "B"))

g_sys_cost_mingen2 <- ggplotGrob(p_sys_cost_mingen + ggtitle("Lower coal minimum generation level") + 
                                  theme(legend.position = c(0, 1), legend.justification = c(0, 1), 
                                        axis.text.x = element_blank()) + labs(tag = "A"))

g_sys_cost_w_storage <- ggplotGrob(p_sys_cost_w_storage + ggtitle("Battery storage") + 
                                     theme(legend.position = c(0, 1), legend.justification = c(0, 1)) +
                                    labs(tag = "C"))

g_sys_cost_w_storage2 <- ggplotGrob(p_sys_cost_w_storage + ggtitle("Battery storage") + 
                                     theme(legend.position = c(0, 1), legend.justification = c(0, 1), axis.text.x = element_blank()) +
                                    labs(tag = "B"))

g_sys_cost_mingen_storage <- ggplotGrob(p_sys_cost_mingen_storage + 
                                          ggtitle("Lower coal minimum generation level & battery storage") + 
                                  theme(legend.position = c(0, 1), legend.justification = c(0, 1)) +
                                   labs(tag = "D"))

g_sys_cost_loadMod <- ggplotGrob(p_sys_cost_loadMod + ggtitle("Urbanized future demand") + 
                                   theme(legend.position = c(0, 1), legend.justification = c(0, 1)) + labs(tag = "D"))

g_sys_cost_loadMod2 <- ggplotGrob(p_sys_cost_loadMod + ggtitle("Urbanized future demand") + 
                                   theme(legend.position = c(0, 1), legend.justification = c(0, 1)) + labs(tag = "C"))

g_sys_cost_coalhc <- ggplotGrob(p_sys_cost_coalhc + ggtitle("High coal costs") + 
                                   theme(legend.position = c(0, 1), legend.justification = c(0, 1)) + labs(tag = "D"))
...
grob_sysCost.plot2 <- 
  arrangeGrob(grobs = 
                list(rbind(g_sys_cost, g_sys_cost_w_storage, size = "last"), 
                     rbind(g_sys_cost_mingen, g_sys_cost_mingen_storage, size = "last")), ncol = 2)

grid.newpage()
grid.draw(grob_sysCost.plot2)

#### Panel 3 ####
grob_sysCost.plot3 <- 
  arrangeGrob(grobs = 
                list(rbind(g_sys_cost_mingen2, g_sys_cost_loadMod2, size = "last"), 
                     rbind(g_sys_cost_w_storage2, g_sys_cost_coalhc, size = "last")), ncol = 2)

grid.newpage()
grid.draw(grob_sysCost.plot3)

# ggsave(file = paste0(output.folder.paper, "add_sys_costs_all_scenarios", "_v1", ".png"), grob_sysCost.plot, width=7, height=5)
#ggsave(file = paste0(output.folder.paper, "add_sys_costs_all_scenarios", "_v2", ".png"), grob_sysCost.plot2, width=8, height=5)
#ggsave(file = paste0(output.folder.paper, "add_sys_costs_all_scenarios", "_v3", ".png"), grob_sysCost.plot3, width=8, height=5)
...
p_cost_emissions_avg1 <- p_cost_emissions_avg + ggtitle("Wind and solar costs") + 
                                     theme(legend.position = c(0, 1), legend.justification = c(0, 1), 
                                           axis.text.x = element_blank())

g_cost_emissions_avg <- ggplotGrob(p_cost_emissions_avg1 + labs(tag = "A")) 

p_cost_emissions_avg_mingen1 <- p_cost_emissions_avg_mingen + ggtitle("Lower coal minimum generation level") +
                                            theme(legend.position = c(0, 1), legend.justification = c(0, 1), 
                                                  axis.text.x = element_blank())

g_cost_emissions_avg_mingen <- ggplotGrob(p_cost_emissions_avg_mingen1 + labs(tag = "B"))
g_cost_emissions_avg_mingen2 <- ggplotGrob(p_cost_emissions_avg_mingen1 + labs(tag = "A"))

p_cost_emissions_avg_w_storage1 <- p_cost_emissions_avg_w_storage + ggtitle("Battery storage") + 
                                               theme(legend.position = c(0, 1), legend.justification = c(0, 1))

g_cost_emissions_avg_w_storage <- ggplotGrob(p_cost_emissions_avg_w_storage1 + labs(tag = "C"))
g_cost_emissions_avg_w_storage2 <- ggplotGrob(p_cost_emissions_avg_w_storage1 + theme(axis.text.x = element_blank()) + labs(tag = "B"))

p_cost_emissions_avg_mingen_storage1 <- p_cost_emissions_avg_mingen_storage + ggtitle("Lower coal minimum generation level & battery storage") +
                                            theme(legend.position = c(0, 1), legend.justification = c(0, 1))

g_cost_emissions_avg_mingen_storage <- ggplotGrob(p_cost_emissions_avg_mingen_storage1 + labs(tag = "D"))

p_cost_emissions_avg_loadMod1 <- p_cost_emissions_avg_loadMod + ggtitle("Future demand profiles") + 
                                             theme(legend.position = c(0, 1), legend.justification = c(0, 1))

g_cost_emissions_avg_loadMod <- ggplotGrob(p_cost_emissions_avg_loadMod1 + labs(tag = "D"))
g_cost_emissions_avg_loadMod2 <- ggplotGrob(p_cost_emissions_avg_loadMod1 + labs(tag = "C"))

p_cost_emissions_avg_coalhc1 <- p_cost_emissions_avg_coalhc + ggtitle("High coal costs") + 
                                             theme(legend.position = c(0, 1), legend.justification = c(0, 1))

g_cost_emissions_avg_coalhc <- ggplotGrob(p_cost_emissions_avg_coalhc1 + labs(tag = "D"))
...
#### Panel 2 ####
grob_emissionsCost.plot1 <- 
  arrangeGrob(grobs = 
                list(rbind(g_cost_emissions_avg, g_cost_emissions_avg_w_storage, size = "last"), 
                     rbind(g_cost_emissions_avg_mingen, g_cost_emissions_avg_mingen_storage, size = "last")), ncol = 2)

grid.newpage()
grid.draw(grob_emissionsCost.plot1)

#ggsave(file = paste0(output.folder.paper, "emissions_costs_all_scenarios", "_v1", ".png"), grob_emissionsCost.plot1, width=8, height=5)

#### Panel 3 - min gen, battery storage, urbanized demand, and high cost coal [USED IN PAPER] ####
grob_emissionsCost.plot2 <- 
  arrangeGrob(grobs = 
                list(rbind(g_cost_emissions_avg_mingen2, g_cost_emissions_avg_loadMod2, size = "last"), 
                     rbind(g_cost_emissions_avg_w_storage2, g_cost_emissions_avg_coalhc, size = "last")), ncol = 2)

grid.newpage()
grid.draw(grob_emissionsCost.plot2)

#ggsave(file = paste0(output.folder.paper, "emissions_costs_all_scenarios", "_v2", ".png"), grob_emissionsCost.plot2, width=8, height=5)
#ggsave(file = paste0(output.folder.paper, "emissions_costs_all_scenarios", "_v2", ".pdf"), grob_emissionsCost.plot2, width=18.7, height=12.5, units = "cm") # This dimension is 17.8/0.95 wide. make that adjustment in Latex
...
grob_emissionsCost.plot5 <- arrangeGrob(arrangeGrob(gA,gB,gC,gD,gE,nrow=5,heights=c(.5, .5, .5, .5, .25)), plot.legend.vre.split, nrow = 2, heights=c(20,0.5))

grid.newpage()
grid.draw(grob_emissionsCost.plot5)

#ggsave(file = paste0(output.folder.paper, "emissions_costs_all_scenarios_vre_split_vertical", "_v1", ".png"), grob_emissionsCost.plot5, width=4, height=9)
```
...
colors.capacity.buildout <- c("#CC79A7", "#D55E00", "#56B4E9", "#0072B2", "#000000", "#CC3333", "#FF6600", "#999999", "#FF9900", "#006699", "#CC3333", "#FF6600", "#999999")
colors.capacity.buildout <- c("#666666", "#FF6600", "#CC3333", "#FF9900", "#006699", "#999999", "#FF6600", "#CC3333", "#000000", "#0072B2", "#56B4E9", "#D55E00", "#CC79A7")
colors.capacity.buildout2 <- c("#FF9900", "#006699", "#666666", "#FF6600", "#CC3333", "#999999", "#FF6600", "#CC3333", "#000000", "#0072B2", "#56B4E9", "#D55E00", "#CC79A7")
...
#####
# Plot the capacities - both existing and new capacities bur VRE on top
capacity.all$technology <- reorder.factor(capacity.all$technology, new.order=positions.tech2)
capacity.all$scenario <- reorder.factor(capacity.all$scenario, new.order=positions3)
capacity.all <- capacity.all[order(-technology)]
                              
p_all_capacity2 <- ggplot(capacity.all, aes(scenario, capacity_GW, fill = technology, order = technology)) +
  geom_bar(stat = "identity", width = width.bar) +
  facet_grid(~scenario_build, scales = "free_x", space = "free_x") +
  labs(y = "Installed Capacity (MW)", x = NULL) +
  scale_fill_manual(values = colors.capacity.buildout2) + 
  #scale_x_discrete(breaks=positions3, labels=share.labels2) +
  scale_x_discrete(breaks=positions3, labels=share.labels.verbose.noVRE) +
  theme(axis.text.x = element_text(angle=90, hjust=1, vjust=0.5), legend.title = element_blank()) +
  background_grid()

p_all_capacity2
ggsave(file = paste0(output.folder.paper, "existing_new_buildout_capacity_base", "_v0", ".png"), p_all_capacity2, width=6, height=4)
#####
...
#### Avoided conventional capacity - absolute and per MW VRE

scenarios.to.include.avoidedCap <- c("base")
scenarios.labels.base <- c("base case")
linetype_base <- c(rep("solid", 2)) #c("solid", "dashed", "dashed")
y_limits_avoidedCap = c(0, 40)
y_limits_avoidedCap_perVREmw = c(0, 0.2)

# Absolute avoided conventional capacity
data.plot <- results.processed[scenario!="S0W0" & scenario_main %in% scenarios.to.include.avoidedCap, .(scenario_main, scenario, scenario_build, avoided_conv_cap_mw)]
data.plot[,avoided_conv_cap_gw:= avoided_conv_cap_mw/1000]
#setnames(data.plot, c("avoided_conv_cap_mw_perVREmw"), c("Avoided New Conventional Capacity"))
data.plot$scenario <- reorder.factor(data.plot$scenario, new.order=positions)
  
p_avoided_conv_cap_absolute <- ggplot(data.plot, aes(scenario, avoided_conv_cap_gw)) +
  geom_hline(color = "grey", yintercept = 0) +
  geom_bar(stat = "identity", width = width.bar) +
  facet_grid(~scenario_build, scales = "free_x", space = "free_x") +
  labs(y = "Avoided Conventional\nCapacity (GW)", x = NULL) +
  scale_fill_manual(values = colors.capacity.buildout) + 
  scale_x_discrete(breaks=positions, labels=share.labels.verbose) +
  theme(axis.text.x = element_text(angle=90, hjust=1, vjust=0.5), legend.title = element_blank(), 
        plot.title = element_text(face="plain"), strip.background = element_rect(colour="black")) +
  scale_y_continuous(limits = y_limits_avoidedCap, sec.axis = sec_axis(~.*1, name = "")) +
  theme(axis.text.y.right = element_blank(), axis.title.y = element_text(margin = margin(t = 0, r = 13, b = 0, l = 0))) +
  background_grid() + labs(tag = "B")

# Avoided conventional capacity per VRE MW
p_avoided_conv_cap_perVREmw <- plot.facet.allLines(results.processed, "avoided_conv_cap_mw_perVREmw", scenarios.to.include.avoidedCap, scenarios.labels.base, positions, colors_RD8, linetype_base,  "", y_limits_avoidedCap_perVREmw, "Avoided Conventional\nCapacity (MW per VRE MW)", "", 1)
p_avoided_conv_cap_perVREmw = p_avoided_conv_cap_perVREmw + theme(axis.text.y.right = element_blank(), legend.position = "none", axis.text.x = element_blank()) + labs(tag = "A")

# Combined plot
p_avoided_conv_cap <- plot_grid(p_avoided_conv_cap_perVREmw, p_avoided_conv_cap_absolute, rel_heights = c(0.8, 1), ncol = 1)

p_avoided_conv_cap
#ggsave(file = paste0(output.folder.paper, "avoided_conventional_capacity", "_v0", ".png"), p_avoided_conv_cap, width=6, height=5)
#ggsave(file = paste0(output.folder.paper, "avoided_conventional_capacity", "_v0", ".pdf"), p_avoided_conv_cap, width=8.7, height=10, units = "cm")
...
### Wind and solar maps, supply curves, and cost trajectories
```{r, wind solar maps costs supply curves, echo=FALSE, , fig.width=7.5, fig.height = 7.5}

#### VRE MAPS
wind_map_fname <- paste0(output.folder.paper, "Maps_and_model_Figures/", "REvalue_selected_sites_200-400-600_wind.png")
solar_map_fname <- paste0(output.folder.paper, "Maps_and_model_Figures/", "REvalue_selected_sites_200-400-600_solarpv.png")
map_wind <- readPNG(wind_map_fname)
map_wind_g <- rasterGrob(map_wind, interpolate=TRUE)
map_solar <- readPNG(solar_map_fname)
map_solar_g <- rasterGrob(map_solar, interpolate=TRUE)

#### VRE SUPPLY CURVES
lcoe_supply_curve_data <- fread(paste0(input.folder, "lcoe_supply_curve_wind_solarPV_data.csv"))
colors.tech <- c("Solar PV" = "#FF9900", "Wind" = "#0072B2")

#lcoe.supply.curve.box.plot.data

p_lcoe.supply.curve.capacity <- ggplot(data = lcoe_supply_curve_data, aes(y=LCOEgenAlt, x = cumInstalledCapacity, group=Tech, color=Tech)) + 
  geom_line(linetype = "solid", size = 1) + 
  #theme_bw() + 
  labs(x="Installed Capacity Potential (GW)", y="Levelized Cost of Electricity \n(USD/MWh)") + theme(legend.position="bottom", legend.title=element_blank()) +
  expand_limits(x=c(0, 1400), y=c(0, 80)) + background_grid() + 
  scale_x_continuous(breaks=c(0, 200, 400, 600, 800,1000,1200, 1400)) + scale_color_manual(values = colors.tech) +
  scale_y_continuous(breaks = c(0, 20, 40, 60, 80), sec.axis = sec_axis(~.*INR_USD/1000, name = "Levelized Cost of Electricity \n(INR/kWh)"))
p_lcoe.supply.curve.capacity

#### VRE COST TRAJECTORIES
linetype_vre_costs <- c("solid", "dashed", "solid", "dashed")
new.vre.costs <- read_excel(paste0(input.folder, "new_projects.xlsx"), sheet = "new_vre_costs")
new.vre.costs <- as.data.table(new.vre.costs)
new.vre.costs[, capital_cost_per_kw := capital_real_cost_per_kw]
new.vre.costs = new.vre.costs[, .SD, .SDcols = c("scenario", "project", "period", "capital_cost_per_kw")]

# new.storage.costs <- read_excel(paste0(input.folder, "new_projects.xlsx"), sheet = "new_battery_costs")
# new.storage.costs <- as.data.table(new.storage.costs)
# new.storage.costs[, capital_cost_per_kw := capital_real_cost_total_per_kw] # selecting column to plot
# # Plot only 4h battery
# new.storage.costs = new.storage.costs[project == "New_Battery_4h", .SD, .SDcols = c("scenario", "project", "period", "capital_cost_per_kw")]
# 
#new.technology.costs <- rbind(new.vre.costs, new.storage.costs)
new.technology.costs <- rbind(new.vre.costs)
# Cost and teechnology labels
new.technology.costs[project == "solarPV", technology := "Solar PV"]
new.technology.costs[project == "wind", technology := "Wind"]
#new.technology.costs[project == "New_Battery_4h", technology := "Battery Storage 4h"]
new.technology.costs[scenario == "VREhigh", scenario_cost := "Base cost"]
new.technology.costs[scenario == "VRElow", scenario_cost := "Low cost"]
#new.technology.costs[scenario == "SThigh", scenario_cost := "High cost"]
#new.technology.costs[scenario == "STlow", scenario_cost := "Low cost"]

p_vre_costs_trajectories <- ggplot(new.technology.costs, aes(x = period, y = capital_cost_per_kw, color = technology, linetype = scenario_cost)) +
  geom_line(size = 1) +
  #facet_wrap(~technology, ncol = 1, scales = "free_x") + 
  labs(y = "Capital cost \n(USD per kW)", x = " ") +
  scale_x_continuous(breaks=c(2018, 2022, 2026, 2030), labels=c(2018, 2022, 2026, 2030)) +
  scale_color_manual(values = c("#FF9900", "#0072B2")) +
  scale_y_continuous(limits = c(0, 1500), sec.axis = sec_axis(~.*INR_USD/1000, name = "Capital cost \n(INR/kW)")) +
  #theme_bw() + 
  theme(legend.title = element_blank(), legend.position = "bottom", legend.direction = "horizontal", legend.justification = 'center') +
  background_grid()

p_vre_costs_trajectories

p_vre_costs_trajectories_legend <- g_legend(p_vre_costs_trajectories)


# Combined plot
#p_vre_maps_curves_costs <- plot_grid(p_supply_curves, p_costs, rel_heights = c(0.5, 0.5), ncol = 2)

p_vre_maps_curves_costs <- plot_grid(plot_grid(map_solar_g, map_wind_g, p_lcoe.supply.curve.capacity + theme(legend.position = "none"), 
                                     p_vre_costs_trajectories + theme(legend.position = "none"), ncol = 2, rel_heights = c(6, 3.75), labels = "AUTO", label_fontface = "plain", label_size = 12),
                                     p_vre_costs_trajectories_legend, nrow = 2, rel_heights = c(10, 0.7))
p_vre_maps_curves_costs

#ggsave(file = paste0(output.folder.paper, "vre_maps_curves_costs", "_v0", ".png"), p_vre_maps_curves_costs, width=7.5, height=7.5)
ggsave(file = paste0(output.folder.paper, "vre_maps_curves_costs", "_v0", ".pdf"), p_vre_maps_curves_costs, width=17.8, height=17.8, units = "cm")
```
...
scenario.main.list <- c("base", "battery30", "battery60", "battery60B50LC")
...
## Paper results - additional system costs over the minimum cost mix within each target and scenario

```{r, results add sys costs over min cost mix, echo = FALSE, results = 'asis'}
...
### additional system costs over the minimum cost mix within each target and scenario
min_values_by_mix_sys_cost_add <- min_values_by_build(results.summary, scenario.main.list, scenario.split.list.nonzero, c("system_cost_add_pMWh_load"))

result.add.sys.cost.over.min.cost.mix <- merge(results.summary[scenario_main %in% scenario.main.list & scenario_split %in% scenario.split.list.nonzero,], min_values_by_mix_sys_cost_add, by = c("scenario_build", "scenario_main"), suffixes = c("", ".min"))

result.add.sys.cost.over.min.cost.mix[, add.sys.cost.over.min.cost.mix.milUSD := (system_cost_add_pMWh_load - system_cost_add_pMWh_load.min) * ann_gen_total_MWh * 10^-6]

result.add.sys.cost.over.min.cost.mix[,c("scenario_main", "scenario_build", "scenario_split", "system_cost_add_pMWh_load", "add.sys.cost.over.min.cost.mix.milUSD")]

write.csv(result.add.sys.cost.over.min.cost.mix[,c("scenario_main", "scenario_build", "scenario_split", "system_cost_add_pMWh_load", "add.sys.cost.over.min.cost.mix.milUSD")], file = paste0(output.folder.processed.results, "results_add_sys_costs_over_min_cost_mix.csv"))

### additional system and emissions costs over the minimum cost mix in base scenario
# add system costs
min_values_by_mix_sys_cost_add <- min_values_by_build(results.summary, c("base"), scenario.split.list.nonzero, c("system_cost_add_pMWh_load"))

result.add.sys.emissions.cost.over.min.cost.mix.base <- merge(results.summary[scenario_main %in% scenario.main.list & scenario_split %in% scenario.split.list.nonzero,], min_values_by_mix_sys_cost_add, by = c("scenario_build"), suffixes = c("", "_min_base"))

result.add.sys.emissions.cost.over.min.cost.mix.base[, add.sys.cost.over.min.cost.mix.base.pMWh.load := (system_cost_add_pMWh_load - system_cost_add_pMWh_load_min_base)]

result.add.sys.emissions.cost.over.min.cost.mix.base[, add.sys.cost.over.min.cost.mix.base.milUSD := (system_cost_add_pMWh_load - system_cost_add_pMWh_load_min_base) * ann_gen_total_MWh * 10^-6]

# add emissions costs over base min mix
min_values_by_mix_emissions_costs <- min_values_by_build(results.summary, c("base"), scenario.split.list.nonzero, c("cost_emissions_reduction_pTonneCO2_avg"))

result.add.sys.emissions.cost.over.min.cost.mix.base <- merge(result.add.sys.emissions.cost.over.min.cost.mix.base
                                                              [scenario_main %in% scenario.main.list 
                                                                & scenario_split %in% scenario.split.list.nonzero,],
                                                              min_values_by_mix_emissions_costs, by = c("scenario_build"),
                                                              suffixes = c("", "_min_base_emissions"))

result.add.sys.emissions.cost.over.min.cost.mix.base[, emissions.cost.over.min.cost.mix.base.pTonneCO2 := (cost_emissions_reduction_pTonneCO2_avg - cost_emissions_reduction_pTonneCO2_avg_min_base_emissions)]

result.add.sys.emissions.cost.over.min.cost.mix.base[,c("scenario_main", "scenario_build", "scenario_split", "system_cost_add_pMWh_load", "add.sys.cost.over.min.cost.mix.base.pMWh.load", "add.sys.cost.over.min.cost.mix.base.milUSD",
                                "cost_emissions_reduction_pTonneCO2_avg", "emissions.cost.over.min.cost.mix.base.pTonneCO2",
                                "total_emissions_reduced_milTonnes", "emissions_reduction")]

write.csv(result.add.sys.emissions.cost.over.min.cost.mix.base[,c("scenario_main", "scenario_build", "scenario_split", "system_cost_add_pMWh_load", "add.sys.cost.over.min.cost.mix.base.pMWh.load", "add.sys.cost.over.min.cost.mix.base.milUSD",
                                "cost_emissions_reduction_pTonneCO2_avg", "emissions.cost.over.min.cost.mix.base.pTonneCO2",
                                "total_emissions_reduced_milTonnes", "emissions_reduction")], file = paste0(output.folder.processed.results, "results_add_sys_emissions_costs_over_min_cost_mix_base.csv"))
...
scenarios.to.compare <- c("base", "battery60")
scenario.comparison <- compare_scenarios_by_build(results.summary, scenarios.to.compare, "system_cost_add_pMWh_load")
scenario.comparison[order(factor(scenario, levels = positions))]

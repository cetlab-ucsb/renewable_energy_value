...
setwd('/Users/rdeshmukh/Dropbox/renewable_energy_value/renewable_energy_value/scripts/')
...
#### Supply curve of existing power plants ####
gen.list.costs.existing <- fread(paste0(input.folder, "gen_all_input_cc_ccgt_diesel", ".csv"))
gen.list.costs.all <- gen.list.costs.existing # just being lazy about changing all variable instances
gen.list.costs.all <- gen.list.costs.all[order(var_cost)] # sort by var cost
gen.list.costs.all[, var_cost_usd := var_cost /INR_USD] # Convert var cost to usd per MWh
gen.list.costs.all[, gen_capacity_GW := gen_capacity/10^3] # Convert generation capacity to GW
#gen.list.costs.all <- gen.list.costs.all[type != "hydro",] # remove dispatchable hydro
...
# Apply outage rates (See python economic dispatch script for outage rates.) # Not derating by outage rates because I am not comparing with demand.
# outage_rate_coal = 0.1 # These outage rates need to match the ones from the conventional buildout algorithm
# outage_rate_gas_ct = 0.1 
# outage_rate_gas_ccgt = 0.1 
# outage_rate_diesel = 0.2 
...
gen.list.costs.all[type == "gas_ct", type := "Gas CT"]
gen.list.costs.all[generator == "NUCLEAR", type := "Nuclear"]
gen.list.costs.all[generator == "HYDRO-STORAGE", type := "Hydro Storage"]
gen.list.costs.all[generator == "HYDRO-ROR", type := "Hydro RoR"]
gen.list.costs.all[generator == "HYDRO-PONDAGE", type := "Hydro RoR"]
gen.list.costs.all[type == "other", type := "Other"]
gen.list.costs.all[type == "diesel", type := "Diesel"]
...
# plot supply curve
colors.conv.gen <- c("#000000", "#ee9944", "#aa6644", "#99bb55", "#66bbbb", "#5588bb", "#444466", "#bb5555")

p_supply_existing <- ggplot(gen.list.costs.all, aes(y = gen_capacity_cumulative, x = var_cost_usd, color = type, stat = "identity")) + geom_point() +
  labs(x = "Variable Cost (USD/MWh)", y = "Generation capacity (GW)" ) +
  scale_color_manual(values = colors.conv.gen) + 
  scale_y_continuous(limits = c(0, 300)) +
  scale_x_continuous(limits = c(0, 160)) +
  theme_bw() + theme(legend.title = element_blank(), panel.grid.minor = element_blank()) + coord_flip()
p_supply_existing

ggsave(file = paste0(output.folder.paper, "Existing_electricity_generation_capacity", "_v0", ".png"), p_supply_existing, width=7, height=5)

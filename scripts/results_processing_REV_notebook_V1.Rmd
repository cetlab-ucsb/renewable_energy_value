...
scenarios <- c("base", "coal_55mingen", "coal_ccgt_0mingen", "wind10LC", "wind20LC", "wind30LC", "solar10LC", "solar20LC", "solar30LC", "wind30LC_solar30LC", "wind120HH_solar1A", "battery15", "battery30", "battery60", "battery15B25LC", "battery30B25LC", "battery15B50LC", "battery30B50LC", "battery60B50LC", "high_cost_coal", "wind30LC_solar30LC_coalHC", "load_modD0M0_energyOnly", "load_modD50M0_energyOnly", "load_modD25M25_energyOnly") 
```
...
**system_cost_add_pMWh_load**: Additional system cost due to VRE for a VRE scenario compared to a zero VRE scenario  per MWH of Load served. (Load served includes battery storage losses in battery scenarios. Additional system cost is divided by ann_gen_total_MWh.)   
...
for (sc in 1:length(scenarios)){
  ...
  # Cost of VRE per MWh of absorbed VRE generation
  ...
  # Cost of battery storage per MWh of absorbed VRE generation
  ...
  # Cost of VRE and battery storage per MWh of absorbed VRE generation
...
scenarios <- c("base", "coal_55mingen", "wind10LC", "wind20LC", "wind30LC", "solar10LC", "solar20LC", "solar30LC", "wind30LC_solar30LC", "wind120HH_solar1A", "battery15", "battery30", "battery60","battery15B25LC", "battery30B25LC", "battery15B50LC", "battery30B50LC", "battery60B50LC", "high_cost_coal", "wind30LC_solar30LC_coalHC")
...
if (re_targets_to_plot == "200-300-400"){
  ...
  y_limits_cost_emissions <- c(-25, 125)
}
...
2) Line plots with facets by VRE split
...
plot.facet.allLines <- function(results.input, main_variable, scenarios.to.include, scenarios.labels, positions, plot.colors, plot.linetype, plot_title, y_limits, y_label, y_label_sec, y_sec_multiplier){
  ...
  q <- ggplot(data.plot, aes(scenario, get(main_variable), group = scenario_main, color = scenario_main, linetype = scenario_main)) +
    ...
    theme(axis.text.x = element_text(angle=90, hjust=1, vjust=1), legend.position="bottom", legend.title = element_blank(), 
          legend.key = element_rect(size=5, fill = "white"), legend.key.size = unit(0.5, 'lines'), legend.key.width = unit(2, 'lines'), 
...
data.plot[, c("wind", "solar") := data.table(str_split_fixed(scenario_split, "-", n=2))]
...
p_vre_split <- ggplot(data.plot, aes(scenario, share_perc, group = technology, fill = technology)) +
  ...
  scale_x_discrete(breaks = positions, labels=share.labels) +
  theme(axis.text.x = element_text(angle=90, hjust=1, vjust=1), legend.title = element_blank()) + scale_y_continuous(labels = percent, sec.axis = sec_axis(trans = ~.*1, name = "", breaks = derive(), labels = derive()))
```
...
scenarios.labels.vre <- c("base wind, solar, coal costs", "low cost wind", "low cost solar", "low cost wind & solar", "low cost wind & solar + high cost coal")
linetype_vre <- c("solid", rep("dashed", 3), "solid")


## Share of VRE
p_vre_share <- plot.facet.allLines(results.processed, "vre_share_after_curt", scenarios.to.include.vre, scenarios.labels.vre, positions, colors_RD7, linetype_vre, "VRE share of total electricity generation", y_limits_vre_share, "", "")
...
## Cost of VRE
p_vre <- plot.facet.allLines(results.processed, "cost_vre_battery_pMWh", scenarios.to.include.vre, scenarios.labels.vre, positions, colors_RD7, linetype_vre, "Average Cost of VRE", y_limits_vre_cost, "USD/MWh of VRE", "INR/kWh of VRE", INR_USD/1000)
p_vre + scale_linetype_manual(labels = scenarios.labels.vre, values = c("solid", rep("dashed", 3), "solid"))

## Capacity value of VRE
p_capacity_value <- plot.facet.allLines(results.processed, "capacity_value_pMWh", scenarios.to.include.vre, scenarios.labels.vre, positions, colors_RD7, linetype_vre, "Average Capacity Value of VRE", y_limits_capacity_value, "USD/MWh of VRE", "INR/kWh of VRE", INR_USD/1000)
p_capacity_value

## Energy value of VRE
p_energy_value <- plot.facet.allLines(results.processed, "energy_value_pMWh", scenarios.to.include.vre, scenarios.labels.vre, positions, colors_RD7, linetype_vre, "Average Energy Value of VRE", y_limits_energy_value, "USD/MWh of VRE", "INR/kWh of VRE", INR_USD/1000)
p_energy_value

## System additional cost of VRE
p_sys_cost <- plot.facet.allLines(results.processed, "system_cost_add_pMWh_load", scenarios.to.include.vre, scenarios.labels.vre, positions, colors_RD7, linetype_vre, "Average Additional Cost of VRE", y_limits_sys_cost, "USD/MWh of Load", "INR/kWh of Load", INR_USD/1000)
p_sys_cost
...
ggsave(file = paste0(output.folder.paper, "wind_solar_coal_costs_all_var", "_v0", ".png"), grob_vre.plot, width=7, height=5)
...
scenarios.to.include.battery <- c("base", "battery30", "battery60", "battery60B50LC")
scenarios.labels.battery <- c("base case - no battery", "battery 30 GW 4h", "battery 60 GW 4h", "battery 60 GW 4h low cost")
linetype_battery <- c("solid", rep("dashed", 2), "solid")

## Share of VRE
p_vre_share_w_storage <- plot.facet.allLines(results.processed, "vre_share_after_curt", scenarios.to.include.battery, scenarios.labels.battery, positions, colors_RD8, linetype_battery,  "VRE share of total electricity generation", y_limits_vre_share, "", "", INR_USD/1000)
# Add vre share without curtailment as a black line
data.plot.to.add <- results.processed[scenario!="S0W0" & scenario_main == "base", .(scenario_main, scenario, scenario_build, vre_share_nocurt)]
#setnames(data.plot, c("V4"), c(eval(main_variable)))
data.plot.to.add$scenario <- reorder.factor(data.plot.to.add$scenario, new.order=positions)
...
p_vre_storage <- plot.facet.allLines(results.processed, "cost_vre_battery_pMWh", scenarios.to.include.battery, scenarios.labels.battery, positions, colors_RD8, linetype_battery,  "Average Cost of VRE and Storage", y_limits_vre_cost, "USD/MWh of VRE", "INR/kWh of VRE", INR_USD/1000)
p_vre_storage

## Capacity value of VRE
p_capacity_value_w_storage <- plot.facet.allLines(results.processed, "capacity_value_pMWh", scenarios.to.include.battery, scenarios.labels.battery, positions, colors_RD8, linetype_battery, "Average Capacity Value of VRE and storage", y_limits_capacity_value, "USD/MWh of VRE", "INR/kWh of VRE", INR_USD/1000)
p_capacity_value_w_storage


## Energy value of VRE
p_energy_value_w_storage <- plot.facet.allLines(results.processed, "energy_value_pMWh", scenarios.to.include.battery, scenarios.labels.battery, positions, colors_RD8, linetype_battery, "Average Energy Value of VRE and storage", y_limits_energy_value, "USD/MWh of VRE", "INR/kWh of VRE", INR_USD/1000)
p_energy_value_w_storage


## System additional cost of VRE
p_sys_cost_w_storage <- plot.facet.allLines(results.processed, "system_cost_add_pMWh_load", scenarios.to.include.battery, scenarios.labels.battery, positions, colors_RD8, linetype_battery, "Average Additional Cost of VRE and storage", y_limits_sys_cost, "USD/MWh of Load", "INR/kWh of Load", INR_USD/1000)
...
ggsave(file = paste0(output.folder.paper, "battery_storage_all_var", "_v0", ".png"), grob_storage.plot, width=7, height=5)
...
linetype_mingen <- c("solid", "dashed", "dashed")

## Share of VRE
p_vre_share_mingen <- plot.facet.allLines(results.processed, "vre_share_after_curt", scenarios.to.include.mingen, scenarios.labels.mingen, positions, colors_RD8, linetype_mingen,  "VRE share of total electricity generation", y_limits_vre_share, "", "", INR_USD/1000)
# Add vre share without curtailment as a black line
data.plot.to.add <- results.processed[scenario!="S0W0" & scenario_main == "base", .(scenario_main, scenario, scenario_build, vre_share_nocurt)]
#setnames(data.plot, c("V4"), c(eval(main_variable)))
data.plot.to.add$scenario <- reorder.factor(data.plot.to.add$scenario, new.order=positions)
...
## Cost of VRE and storage
p_vre_mingen <- plot.facet.allLines(results.processed, "cost_vre_battery_pMWh", scenarios.to.include.mingen, scenarios.labels.mingen, positions, colors_RD8, linetype_mingen,  "Average Cost of VRE", y_limits_vre_cost, "USD/MWh of VRE", "INR/kWh of VRE", INR_USD/1000)
p_vre_mingen

## Capacity value of VRE
p_capacity_value_mingen <- plot.facet.allLines(results.processed, "capacity_value_pMWh", scenarios.to.include.mingen, scenarios.labels.mingen, positions, colors_RD8, linetype_mingen, "Average Capacity Value of VRE", y_limits_capacity_value, "USD/MWh of VRE", "INR/kWh of VRE", INR_USD/1000)
p_capacity_value_mingen


## Energy value of VRE
p_energy_value_mingen <- plot.facet.allLines(results.processed, "energy_value_pMWh", scenarios.to.include.mingen, scenarios.labels.mingen, positions, colors_RD8, linetype_mingen, "Average Energy Value of VRE", y_limits_energy_value, "USD/MWh of VRE", "INR/kWh of VRE", INR_USD/1000)
p_energy_value_mingen


## System additional cost of VRE
p_sys_cost_mingen <- plot.facet.allLines(results.processed, "system_cost_add_pMWh_load", scenarios.to.include.mingen, scenarios.labels.mingen, positions, colors_RD8, linetype_mingen, "Average Additional Cost of VRE", y_limits_sys_cost, "USD/MWh of Load", "INR/kWh of Load", INR_USD/1000)
p_sys_cost_mingen

## Cost of CO2 emissions mitigation
p_cost_emissions_avg_mingen <- plot.facet.allLines(results.processed, "cost_emissions_reduction_pTonneCO2_avg", scenarios.to.include.mingen, scenarios.labels.mingen, positions, colors_RD8, linetype_mingen, expression(Cost~of~CO[2]~Emissions~Mitigation), y_limits_cost_emissions, expression(USD/tonne~CO[2]), expression(INR/kg~CO[2]), INR_USD/1000)
...
```{r, minimum generation level panel, echo=FALSE, message = FALSE}
...
ggsave(file = paste0(output.folder.paper, "min_gen_levels_all_var", "_v0", ".png"), grob_mingen.plot, width=7, height=5)
...
linetype_loadMod <- c("solid", "dashed", "dashed")

## Share of VRE
p_vre_share_loadMod <- plot.facet.allLines(results.processed, "vre_share_after_curt", scenarios.to.include.loadMod, scenarios.labels.loadMod, positions, colors_RD8, linetype_loadMod,  "VRE share of total electricity generation", y_limits_vre_share, "", "", INR_USD/1000)
# Add vre share without curtailment as a black line
data.plot.to.add <- results.processed[scenario!="S0W0" & scenario_main == "base", .(scenario_main, scenario, scenario_build, vre_share_nocurt)]
#setnames(data.plot, c("V4"), c(eval(main_variable)))
data.plot.to.add$scenario <- reorder.factor(data.plot.to.add$scenario, new.order=positions)
...
## Cost of VRE and storage
p_vre_loadMod <- plot.facet.allLines(results.processed, "cost_vre_battery_pMWh", scenarios.to.include.loadMod, scenarios.labels.loadMod, positions, colors_RD8, linetype_loadMod,  "Average Cost of VRE", y_limits_vre_cost, "USD/MWh of VRE", "INR/kWh of VRE", INR_USD/1000)
p_vre_loadMod

## Capacity value of VRE
p_capacity_value_loadMod <- plot.facet.allLines(results.processed, "capacity_value_pMWh", scenarios.to.include.loadMod, scenarios.labels.loadMod, positions, colors_RD8, linetype_loadMod, "Average Capacity Value of VRE", y_limits_capacity_value, "USD/MWh of VRE", "INR/kWh of VRE", INR_USD/1000)
p_capacity_value_loadMod


## Energy value of VRE
p_energy_value_loadMod <- plot.facet.allLines(results.processed, "energy_value_pMWh", scenarios.to.include.loadMod, scenarios.labels.loadMod, positions, colors_RD8, linetype_loadMod, "Average Energy Value of VRE", y_limits_energy_value, "USD/MWh of VRE", "INR/kWh of VRE", INR_USD/1000)
p_energy_value_loadMod


## System additional cost of VRE
p_sys_cost_loadMod <- plot.facet.allLines(results.processed, "system_cost_add_pMWh_load", scenarios.to.include.loadMod, scenarios.labels.loadMod, positions, colors_RD8, linetype_loadMod, "Average Additional Cost of VRE", y_limits_sys_cost, "USD/MWh of Load", "INR/kWh of Load", INR_USD/1000)
p_sys_cost_loadMod

## Cost of CO2 emissions mitigation
p_cost_emissions_avg_loadMod <- plot.facet.allLines(results.processed, "cost_emissions_reduction_pTonneCO2_avg", scenarios.to.include.loadMod, scenarios.labels.loadMod, positions, colors_RD8, linetype_loadMod, expression(Cost~of~CO[2]~Emissions~Mitigation), y_limits_cost_emissions, expression(USD/tonne~CO[2]), expression(INR/kg~CO[2]), INR_USD/1000)
...
```{r, load modified panel, echo=FALSE, message = FALSE}
...
ggsave(file = paste0(output.folder.paper, "modified_loads_all_var", "_v0", ".png"), grob_loadMod.plot, width=7, height=5)
...
ggsave(file = paste0(output.folder.paper, "add_sys_costs_all_scenarios", "_v0", ".png"), grob_sysCost.plot, width=7, height=5)
...
g_cost_emissions_avg <- ggplotGrob(p_cost_emissions_avg + ggtitle("Wind, solar, and coal costs") + 
                                     theme(legend.position = c(0, 1), legend.justification = c(0, 1), 
                                           axis.text.x = element_blank())) 

g_cost_emissions_avg_mingen <- ggplotGrob(p_cost_emissions_avg_mingen + ggtitle("Conventional generator minimum generation levels") +
                                            theme(legend.position = c(0, 1), legend.justification = c(0, 1), 
                                                  axis.text.x = element_blank()))

g_cost_emissions_avg_w_storage <- ggplotGrob(p_cost_emissions_avg_w_storage + ggtitle("Battery storage") + 
                                               theme(legend.position = c(0, 1), legend.justification = c(0, 1)))

g_cost_emissions_avg_loadMod <- ggplotGrob(p_cost_emissions_avg_loadMod + ggtitle("Future demand profiles") + 
                                             theme(legend.position = c(0, 1), legend.justification = c(0, 1)))
...
g_vre_split1 <- ggplotGrob(p_vre_split + theme(legend.position = "none", axis.text.x = element_blank(), axis.title.x = element_blank(), strip.background = element_blank(), strip.text.x = element_blank(), axis.text.y.right = element_blank(), axis.title.y.right = element_text(size = 26), axis.ticks.x.bottom = element_blank(), axis.ticks.y.right = element_blank()))

g_vre_split2 <- ggplotGrob(p_vre_split + labs(y = "") + theme(legend.position = "none", axis.text.x = element_blank(), axis.title.x = element_blank(),strip.background = element_blank(),
...
g_cost_emissions_avg_w_storage_no_xlabel <- ggplotGrob(p_cost_emissions_avg_w_storage + ggtitle("Battery storage") + theme(legend.position = c(0, 1), legend.justification = c(0, 1), axis.text.x = element_blank()))

g_cost_emissions_avg_loadMod_no_xlabel <- ggplotGrob(p_cost_emissions_avg_loadMod + ggtitle("Future demand profiles") + theme(legend.position = c(0, 1), legend.justification = c(0, 1), axis.text.x = element_blank()))
...
#grid.arrange(grob_emissionsCost.plot2, arrangeGrob(g_vre_split1, g_vre_split2, ncol = 2), heights = c(10, 2))
...
ggsave(file = paste0(output.folder.paper, "emissions_costs_all_scenarios_vre_split", "_v0", ".png"), grob_emissionsCost.plot3, width=7, height=5)
...
plot_comparison_byMonth_byHour_line(load_india_2030_forecasts[Day <=365,], columns.to.plot, labels.series, "Hourly average load (GW)", colors_RD8, linetype_RD)

# Cumulative load distribution curves
plot_cum_load_distribution_curves(load_india_2030_forecasts[Day <=365,], columns.to.plot, labels.series, "Hourly load (GW)", colors_RD8, linetype_RD_dotted)

...
scenarios <- c("base", "coal_55mingen", "coal_ccgt_0mingen", "wind10LC", "wind20LC", "wind30LC", "solar10LC", "solar20LC", "solar30LC", "wind30LC_solar30LC", "wind120HH_solar1A", "battery15", "battery30", "battery15B25LC", "battery30B25LC", "battery15B50LC", "battery30B50LC", "high_cost_coal", "wind30LC_solar30LC_coalHC", "load_modD0M0_energyOnly", "load_modD50M0_energyOnly", "load_modD25M25_energyOnly") 
```
...
plot.allLines.byVREsplit <- function(results.input, main_variable, scenarios.to.include, scenarios.labels, positions, plot.colors, plot.linetype, plot_title, y_limits, y_label, y_label_sec, y_sec_multiplier){
  ...
  q <- ggplot(data.plot, aes(scenario_build, get(main_variable), group = interaction(scenario_split, scenario_main), color = scenario_main, linetype = scenario_split)) +
  geom_line(size=0.5) +
  labs(y = y_label, x = NULL) +
  ...
  #scale_x_discrete(breaks=positions, labels=share.labels) +
    ggtitle(plot_title) +
  theme(axis.text.x = element_text(angle=0, hjust=0.5, vjust=1), legend.position="bottom", legend.title = element_blank(), legend.key = element_rect(size=5, fill = "white"), legend.key.size = unit(0.5, 'lines'), legend.key.width = unit(2, 'lines'), plot.title = element_text(face="plain")) +
  scale_y_continuous(limits = y_limits, sec.axis = sec_axis(~.*y_sec_multiplier, name = y_label_sec)) +
  background_grid()
...
linetype_vre <- c("solid", rep("dotted", 2), "dashed", "solid")
...
facet_wrap(~scenario_build, scales = "free_x") +
scale_y_continuous(limits = y_limits_vre_share, labels = percent)
...
### Battery storage plots -- various formats -- Delete?
...
scenarios.labels.battery <- c("base case - no battery", "battery 15 GW 4h", "battery 30 GW 4h", "battery 30 GW 4h low cost")
linetype_battery <- c("solid", rep("dashed", 2), "dotdash")

## Share of VRE
p_vre_share_w_storage <- plot.facet.allLines(results.processed, "vre_share_after_curt", scenarios.to.include.battery, scenarios.labels.battery, positions, colors_RD7, linetype_battery,  "VRE share of total electricity generation", y_limits_vre_share, "", "", INR_USD/1000)
# Add vre share without curtailment as a black line
data.plot.to.add <- results.processed[scenario!="S0W0" & scenario_main == "base", .(scenario_main, scenario, scenario_build, vre_share_nocurt)]
#setnames(data.plot, c("V4"), c(eval(main_variable)))
data.plot.to.add$scenario <- reorder.factor(data.plot.to.add$scenario, new.order=positions)
...
p_vre_storage <- plot.facet.allLines(results.processed, "cost_vre_battery_pMWh", scenarios.to.include.battery, scenarios.labels.battery, positions, colors_RD7, linetype_battery,  "Average Cost of VRE and Storage", y_limits_vre_cost, "USD/MWh of VRE", "INR/kWh of VRE", INR_USD/1000)
p_vre_storage

## Capacity value of VRE
p_capacity_value_w_storage <- plot.facet.allLines(results.processed, "capacity_value_pMWh", scenarios.to.include.battery, scenarios.labels.battery, positions, colors_RD7, linetype_battery, "Average Capacity Value of VRE and storage", y_limits_capacity_value, "USD/MWh of VRE", "INR/kWh of VRE", INR_USD/1000)
p_capacity_value_w_storage


## Energy value of VRE
p_energy_value_w_storage <- plot.facet.allLines(results.processed, "energy_value_pMWh", scenarios.to.include.battery, scenarios.labels.battery, positions, colors_RD7, linetype_battery, "Average Energy Value of VRE and storage", y_limits_energy_value, "USD/MWh of VRE", "INR/kWh of VRE", INR_USD/1000)
p_energy_value_w_storage


## System additional cost of VRE
p_sys_cost_w_storage <- plot.facet.allLines(results.processed, "system_cost_add_pMWh_load", scenarios.to.include.battery, scenarios.labels.battery, positions, colors_RD7, linetype_battery, "Average Additional Cost of VRE and storage", y_limits_sys_cost, "USD/MWh of Load", "INR/kWh of Load", INR_USD/1000)
...
linetype_mingen <- c("solid", "dashed", "dotdash")

## Share of VRE
p_vre_share_mingen <- plot.facet.allLines(results.processed, "vre_share_after_curt", scenarios.to.include.mingen, scenarios.labels.mingen, positions, colors_RD7, linetype_mingen,  "VRE share of total electricity generation", y_limits_vre_share, "", "", INR_USD/1000)
# Add vre share without curtailment as a black line
data.plot.to.add <- results.processed[scenario!="S0W0" & scenario_main == "base", .(scenario_main, scenario, scenario_build, vre_share_nocurt)]
#setnames(data.plot, c("V4"), c(eval(main_variable)))
data.plot.to.add$scenario <- reorder.factor(data.plot.to.add$scenario, new.order=positions)
...
## Cost of VRE and storage
p_vre_mingen <- plot.facet.allLines(results.processed, "cost_vre_battery_pMWh", scenarios.to.include.mingen, scenarios.labels.mingen, positions, colors_RD7, linetype_mingen,  "Average Cost of VRE", y_limits_vre_cost, "USD/MWh of VRE", "INR/kWh of VRE", INR_USD/1000)
p_vre_mingen

## Capacity value of VRE
p_capacity_value_mingen <- plot.facet.allLines(results.processed, "capacity_value_pMWh", scenarios.to.include.mingen, scenarios.labels.mingen, positions, colors_RD7, linetype_mingen, "Average Capacity Value of VRE", y_limits_capacity_value, "USD/MWh of VRE", "INR/kWh of VRE", INR_USD/1000)
p_capacity_value_mingen


## Energy value of VRE
p_energy_value_mingen <- plot.facet.allLines(results.processed, "energy_value_pMWh", scenarios.to.include.mingen, scenarios.labels.mingen, positions, colors_RD7, linetype_mingen, "Average Energy Value of VRE", y_limits_energy_value, "USD/MWh of VRE", "INR/kWh of VRE", INR_USD/1000)
p_energy_value_mingen


## System additional cost of VRE
p_sys_cost_mingen <- plot.facet.allLines(results.processed, "system_cost_add_pMWh_load", scenarios.to.include.mingen, scenarios.labels.mingen, positions, colors_RD7, linetype_mingen, "Average Additional Cost of VRE", y_limits_sys_cost, "USD/MWh of Load", "INR/kWh of Load", INR_USD/1000)
p_sys_cost_mingen

## Cost of CO2 emissions mitigation
p_cost_emissions_avg_mingen <- plot.facet.allLines(results.processed, "cost_emissions_reduction_pTonneCO2_avg", scenarios.to.include.mingen, scenarios.labels.mingen, positions, colors_RD7, linetype_mingen, expression(Cost~of~CO[2]~Emissions~Mitigation), y_limits_cost_emissions, expression((USD/tonne~CO[2])), expression((INR/kg~CO[2])), INR_USD/1000)
...
## Share of VRE
p_vre_share_loadMod <- plot.facet.allLines(results.processed, "vre_share_after_curt", scenarios.to.include.loadMod, scenarios.labels.loadMod, positions, colors_RD7, linetype_loadMod,  "VRE share of total electricity generation", y_limits_vre_share, "", "", INR_USD/1000)
# Add vre share without curtailment as a black line
data.plot.to.add <- results.processed[scenario!="S0W0" & scenario_main == "base", .(scenario_main, scenario, scenario_build, vre_share_nocurt)]
#setnames(data.plot, c("V4"), c(eval(main_variable)))
data.plot.to.add$scenario <- reorder.factor(data.plot.to.add$scenario, new.order=positions)
...
## Cost of VRE and storage
p_vre_loadMod <- plot.facet.allLines(results.processed, "cost_vre_battery_pMWh", scenarios.to.include.loadMod, scenarios.labels.loadMod, positions, colors_RD7, linetype_loadMod,  "Average Cost of VRE", y_limits_vre_cost, "USD/MWh of VRE", "INR/kWh of VRE", INR_USD/1000)
p_vre_loadMod

## Capacity value of VRE
p_capacity_value_loadMod <- plot.facet.allLines(results.processed, "capacity_value_pMWh", scenarios.to.include.loadMod, scenarios.labels.loadMod, positions, colors_RD7, linetype_loadMod, "Average Capacity Value of VRE", y_limits_capacity_value, "USD/MWh of VRE", "INR/kWh of VRE", INR_USD/1000)
p_capacity_value_loadMod


## Energy value of VRE
p_energy_value_loadMod <- plot.facet.allLines(results.processed, "energy_value_pMWh", scenarios.to.include.loadMod, scenarios.labels.loadMod, positions, colors_RD7, linetype_loadMod, "Average Energy Value of VRE", y_limits_energy_value, "USD/MWh of VRE", "INR/kWh of VRE", INR_USD/1000)
p_energy_value_loadMod


## System additional cost of VRE
p_sys_cost_loadMod <- plot.facet.allLines(results.processed, "system_cost_add_pMWh_load", scenarios.to.include.loadMod, scenarios.labels.loadMod, positions, colors_RD7, linetype_loadMod, "Average Additional Cost of VRE", y_limits_sys_cost, "USD/MWh of Load", "INR/kWh of Load", INR_USD/1000)
p_sys_cost_loadMod

## Cost of CO2 emissions mitigation
p_cost_emissions_avg_loadMod <- plot.facet.allLines(results.processed, "cost_emissions_reduction_pTonneCO2_avg", scenarios.to.include.loadMod, scenarios.labels.loadMod, positions, colors_RD7, linetype_loadMod, expression(Cost~of~CO[2]~Emissions~Mitigation), y_limits_cost_emissions, expression((USD/tonne~CO[2])), expression((INR/kg~CO[2])), INR_USD/1000)
...
g_sys_cost <- ggplotGrob(p_sys_cost + theme(legend.position = c(0, 1), legend.justification = c(0, 1), axis.text.x = element_blank()))

g_sys_cost_mingen <- ggplotGrob(p_sys_cost_mingen + theme(legend.position = c(0, 1), legend.justification = c(0, 1), axis.text.x = element_blank()))

g_sys_cost_w_storage <- ggplotGrob(p_sys_cost_w_storage + theme(legend.position = c(0, 1), legend.justification = c(0, 1)))

g_sys_cost_loadMod <- ggplotGrob(p_sys_cost_loadMod + theme(legend.position = c(0, 1), legend.justification = c(0, 1)))
...
load_india_2030_forecasts <- fread(paste0(input.folder, "load_forecasts/load2030_19EPS_mod_all.csv"))
...
plot_comparison_byMonth_byHour_line(load_india_2030_forecasts[Day <=365,], columns.to.plot, labels.series, "Hourly average load (GW)", colors_RD7, linetype_RD)

# Cumulative load distribution curves
plot_cum_load_distribution_curves(load_india_2030_forecasts[Day <=365,], columns.to.plot, labels.series, "Hourly load (GW)", colors_RD7, linetype_RD_dotted)

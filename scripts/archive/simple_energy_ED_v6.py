...
outage_rate_coal = 0.2 # These outage rates need to match the ones from the conventional buildout algorithm
outage_rate_gas_ct = 0.2 # These outage rates need to match the ones from the conventional buildout algorithm
outage_rate_gas_ccgt = 0.2 # These outage rates need to match the ones from the conventional buildout algorithm
...
#############################################
'''
scenarios = ["S0W600", "S300W300", "S600W0", "S0W400", "S200W200", "S400W0", "S0W200", "S100W100", "S200W0"]#, "S300W100"] # List of VRE scenarios ["S0W200", "S50W150", "S100W100", "S150W50", "S200W0"]
...
#        model.Enforce_Min_Dispatch_Limit_Diesel = Constraint(model.TIMEPOINTS, model.GENDIESEL, rule=Enforce_Min_Dispatch_Limit_Diesel_rule)
    ...
    ## SUMMARY TABLE
    scenarioSummary = pd.DataFrame([[scenarios[sc] + scenario_suffix, dispatch_cost_annual, dispatch_annual_gen_all,
    ...
    if any(results_all_scenarios.scenario == scenarios[sc] + scenario_suffix):
        print "Deleted scenario's old result"
        results_all_scenarios = results_all_scenarios[results_all_scenarios.scenario != scenarios[sc] + scenario_suffix]
    ...
    dispatch_all_ts_annual.to_csv(outputPath + "dispatch_all_gen_" + scenarios[sc] + scenario_suffix + ".csv", sep=',', index = True)

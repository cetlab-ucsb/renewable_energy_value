...
genMOCK_csv = "gen_mock_input_conv_buildout.csv" # List of mock generators to determine buildout
...
#############################################
'''
scenarios = ["S0W400"]#, "S300W100"] # List of VRE scenarios ["S0W200", "S50W150", "S100W100", "S150W50", "S200W0"]
...
#                                        "ann_gen_vre_nocurt_MWh", "ann_gen_coal_MWh", "ann_gen_gas_MWh", "ann_gen_other_MWh",
#                                        "new_capacity_coal_MW", "new_capacity_gas_MW", "capacity_vre_MW", "capacity_solarPV_MW", "capacity_wind_MW", "date_time"])
#

# Path for output files for conventional generator builout
outputConvBuildoutPath = inputPath + "new_conventional_capacity\\"
...
#    
    ...
    ## MOCK Generators
    genMOCK_toAdd = pd.read_csv(inputPath + genMOCK_csv, sep=',')
    ...
    # Add the new mock generators for the current scenario
    #genALL_input = genALL_input.append(genCONV_toAdd, ignore_index=True)
    
    
    ## Combine Mock and Hydro Storage gnerators
    genMOCK_HYDRO_input = genALL_input[genALL_input['type'].isin(['hydro'])].append(genMOCK_toAdd, ignore_index=True)
...
#        genALL_input.ix[genALL_input.type == 'gas', 'gen_capacity'] = genALL_input.ix[genALL_input.type == 'gas', 'gen_capacity'] * (1-outage_rate_gas)
#        genALL_input.ix[genALL_input.type == 'other', 'gen_capacity'] = genALL_input.ix[genALL_input.type == 'other', 'gen_capacity'] * (1-outage_rate_other)
    ...
    genMOCK_HYDRO = genMOCK_HYDRO_input['generator'].tolist()

    # Variable cost
    genALL_varCost = genMOCK_HYDRO_input[['generator', 'var_cost']]
    genALL_varCost.set_index('generator', inplace=True)
    genALL_varCost = genALL_varCost.to_dict()['var_cost']    
    # Capacity - # All MOCK and HYDRO generators
    genALL_genCapacity = genMOCK_HYDRO_input[['generator', 'gen_capacity']]
...
#        
        ...
        model.GEN = Set(initialize=genMOCK_HYDRO) # Initialize with the list of all generators
...
#        genCOAL_minGen_cf = 0.55 ## For now, the entire COAL fleet will have the same minimum CF, derated by the outage rate.
...
#        genOTHER_minGen_cf = 0.70 ## For now, the entire OTHER fleet will have the same minimum CF.
        ...
        model.net_load_mw = Param(model.TIMEPOINTS, default=net_load_ts_dict)
...
#        
        ...
        def Conservation_Of_Energy_rule(mod, t):
            return sum(mod.DispatchMW[t, gen] for gen in mod.GEN) == mod.net_load_mw[t] 
        ...
        def Enforce_Max_Dispatch_Limit_rule(mod, t, gen):
            return mod.DispatchMW[t, gen] <= mod.gen_capacity[gen]
...
#        model.Enforce_Min_Dispatch_Limit_Gas = Constraint(model.TIMEPOINTS, model.GENGAS, rule=Enforce_Min_Dispatch_Limit_Gas_rule)
#        
...
#        
        ...
        # DISPATCH OF HYDRO STORAGE
        p = "HYDRO-STORAGE"
        dispatch_hydro_ts = [model_instance.DispatchMW[t,p].value for t in timepoints]
        dispatch_hydro_ts = pd.DataFrame({p : dispatch_ts_gen}, index = timepoints)
        
        
        dispatch_net_load_ts_df = pd.concat([net_load_ts, dispatch_hydro_ts], axis=1)
        dispatch_net_load_ts_df['net_load_final'] =  dispatch_net_load_ts_df['net_load'] - dispatch_net_load_ts_df['HYDRO-STORAGE']
        dispatch_net_load_ts_df = dispatch_net_load_ts_df.round(0)
...
#        dispatch_gas_ts = pd.DataFrame(index = timepoints)
#        for p in genGAS:
#            dispatch_ts_gen = [model_instance.DispatchMW[t,p].value for t in timepoints]
#            dispatch_ts_gen_df = pd.DataFrame({p : dispatch_ts_gen}, index = timepoints)
#            # dispatch_ts = dispatch_ts.concat(dispatch_ts_gen_df, axis = 1)
#            dispatch_gas_ts[p] = dispatch_ts_gen_df
#        dispatch_annual_gen_gas = np.around(dispatch_annual_gen_gas + dispatch_gas_ts.sum(axis=1).sum(axis=0), decimals = 2) # Total annual generation
#        
...
#        dispatch_all_gas_ts = pd.DataFrame(dispatch_gas_ts.sum(axis=1), columns = ['Gas'])
#        dispatch_all_other_ts = pd.DataFrame(dispatch_other_ts.sum(axis=1), columns = ['Other'])
#        dispatch_all_gen_ts = pd.concat([dispatch_non_fossil_ts, dispatch_all_coal_ts, dispatch_all_gas_ts, dispatch_all_other_ts], axis = 1)
#        
        #dispatch_all_hydro_ts = dispatch_all_ts["HYDRO-STORAGE"]
...
#    new_capacity_gas = genCONV_toAdd['gen_capacity'][genCONV_toAdd['type']=='gas'].sum()
#    
...
#    
#    ## SUMMARY TABLE
#    scenarioSummary = pd.DataFrame([[scenarios[sc], dispatch_cost_annual, dispatch_annual_gen_all,
#                                     dispatch_annual_gen_vre, dispatch_annual_gen_solarPV, dispatch_annual_gen_wind,
#                                     potential_gen_vre, dispatch_annual_gen_coal, dispatch_annual_gen_gas, dispatch_annual_gen_other,
#                                     new_capacity_coal, new_capacity_gas, capacity_vre, capacity_solarPV, capacity_wind, strftime("%Y-%m-%d %H:%M:%S")]], 
...
#    
#    if any(results_all_scenarios.scenario == scenarios[sc]):
#        print "Deleted scenario's old result"
#        results_all_scenarios = results_all_scenarios[results_all_scenarios.scenario != scenarios[sc]]
#        
...
#    dispatch_all_ts_annual.to_csv(outputPath + "dispatch_all_gen_" + scenarios[sc] + ".csv", sep=',', index = True)

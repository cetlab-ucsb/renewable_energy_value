...
outage_rate_coal = 0.2 # These outage rates need to match the ones from the max net peak algorithm
outage_rate_gas = 0.3 # These outage rates need to match the ones from the max net peak algorithm
outage_rate_other = 0.3 # These outage rates need to match the ones from the max net peak algorithm
...
#############################################
'''
scenarios = ["S400W0", "S300W100"] # List of VRE scenarios ["S0W200", "S50W150", "S100W100", "S150W50", "S200W0"]
...
for sc in range(len(scenarios)):
    ...
    genCONV_toAdd = pd.read_csv(inputPathNEWCONV_capacity + str(yearAnalysis) + "_conventional_capacity_" + scenarios[sc] + ".csv", sep=',')
